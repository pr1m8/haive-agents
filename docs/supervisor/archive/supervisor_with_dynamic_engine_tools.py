...
class DynamicToolSupervisor(ReactAgent):
    ...
    def _create_base_tools(self) -> list[BaseTool]:
        ...
        def check_agent_capability(agent_name: str) -> dict[str, Any]:
            ...
            return {'name': agent_name, 'description': metadata.get('description', 'No description'), 'available': True}
    ...
    def _update_choice_model(self):
        ...
        if agent_names:
            self.agent_choice_model = DynamicChoiceModel.from_choices(agent_names, name='AgentSelection', description='Available agents to execute')
...
async def test_dynamic_engine_tools():
    ...
    supervisor_engine = AugLLMConfig(name='supervisor_engine', model='gpt-4', tools=[], system_message='You are a task supervisor that manages other agents.\n\nAvailable tools:\n- list_available_agents: See what agents are registered\n- check_agent_capability: Check what a specific agent can do\n- execute_[agent_name]: Execute a specific agent (tools added dynamically)\n\nAlways check available agents before trying to execute tasks.').create()
    ...
    math_engine = AugLLMConfig(name='math_engine', model='gpt-4', tools=[calculate], system_message='You are a math assistant. Use the calculate tool for computations.').create()
    math_agent = SimpleAgent(name='math_agent', engine=math_engine)
    supervisor.register_agent('math_agent', math_agent, 'Mathematical calculations and computations')
    await supervisor.arun('Calculate 15 * 4 + 10')
    search_engine = AugLLMConfig(name='search_engine', model='gpt-4', system_message='You are a search assistant. Simulate searching for information and return relevant results.').create()
    ...
    await supervisor.arun('Search for the height of Mount Everest in meters, then calculate how many feet that is (1 meter = 3.28084 feet)')
