...
class SupervisorStateWithTools(SupervisorState):
    ...
    def validate_agent_routing(self):
        ...
        if self.next_agent is not None and self.next_agent != 'END':
            if self.next_agent not in self.agents:
                ...
                raise ValueError(f"Agent '{self.next_agent}' not found in registry. Available: {available}")
        return self
    ...
    def create_handoff_tool(self, agent_name: str):
        ...
        decorated_tool = tool(description=f'Hand off a task to {agent_name}. {agent_info.description}')(handoff_tool)
    ...
    def create_choice_tool(self):
        ...
        return tool(choose_agent, description='Choose which agent should handle a task using validated selection')
