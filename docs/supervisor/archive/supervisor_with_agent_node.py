...
class AgentExecutionNode:
    ...
    async def __call__(self, state: SupervisorState) -> dict[str, Any]:
        ...
        try:
            ...
            state.messages.append({'role': 'assistant', 'content': result, 'agent': state.selected_agent, 'timestamp': datetime.now().isoformat()})
...
class SupervisorWithAgentNode(ReactAgent):
    ...
    def _create_routing_tools(self):
        ...
        tools.extend([select_math_agent, select_search_agent, select_writer_agent, list_available_agents, check_agent_status])
    ...
    def build_graph(self) -> BaseGraph:
        ...
        graph.add_conditional_edges('supervisor', self._check_agent_selection, {'execute': 'agent_node', 'end': END})
...
async def demo_agent_node_pattern():
    ...
    def multiply(a: int, b: int) -> int:
        ...
        return a * b
    math_engine = AugLLMConfig(name='math_engine', model='gpt-4', tools=[multiply], system_message='You are a math assistant.').create()
    math_agent = SimpleAgent(name='math_agent', engine=math_engine)
    search_engine = AugLLMConfig(name='search_engine', model='gpt-4', system_message='You are a search assistant. Simulate searching.').create()
    search_agent = SimpleAgent(name='search_agent', engine=search_engine)
    supervisor_engine = AugLLMConfig(name='supervisor_engine', model='gpt-4', tools=[], system_message='You are a task routing supervisor.').create()
    supervisor = SupervisorWithAgentNode(name='supervisor', engine=supervisor_engine, state_schema=SupervisorState)
    initial_state = SupervisorState(available_agents={'math_agent': math_agent, 'search_agent': search_agent})
