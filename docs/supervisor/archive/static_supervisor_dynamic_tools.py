...
class StaticSupervisorWithDynamicTools(ReactAgent):
    ...
    def _create_base_tools(self):
        ...
        def check_agent_status(agent_name: str) -> dict[str, Any]:
            ...
            if agent_name in self.agent_registry.agents:
                ...
                return {'name': agent_name, 'available': True, 'description': metadata.get('description', 'No description'), 'registered_at': str(metadata.get('registered_at', 'Unknown'))}
    ...
    def _sync_agent_tools(self):
        ...
        self.engine.tools = [t for t in self.engine.tools if t.name in base_tool_names or not t.name.startswith('execute_')]
        new_tools = []
    ...
    def _update_choice_model(self):
        ...
        if agent_names:
            self.agent_choice_model = DynamicChoiceModel.from_choices(agent_names, name='AvailableAgents', description='Currently available agents')
...
async def demo_static_dynamic_tools():
    ...
    supervisor_engine = AugLLMConfig(name='supervisor_engine', model='gpt-4', tools=[], system_message='You are a task supervisor.\nUse list_available_agents to see what agents are available.\nUse execute_[agent_name] tools to delegate tasks to specific agents.\nAlways check available agents before trying to execute.').create()
    ...
    def add(a: int, b: int) -> int:
        """Add two numbers."""
        return a + b
    math_engine = AugLLMConfig(name='math_engine', model='gpt-4', tools=[add], system_message='You are a math helper. Use the add tool for calculations.').create()
    ...
    search_engine = AugLLMConfig(name='search_engine', model='gpt-4', system_message='You are a search helper. Simulate searching for information.').create()
