...
class DynamicAgentRegistry:
    ...
    def register(self, name: str, agent: Any, description: str, capabilities: list[str], active: bool=True):
        """Register an agent with metadata."""
        self.agents[name] = agent
        self.agent_metadata[name] = {'description': description, 'capabilities': capabilities, 'registered_at': datetime.now(), 'activation_count': 0}
        if active:
    ...
    def get_active_agents(self) -> dict[str, Any]:
        ...
        return {name: {'agent': self.agents[name], 'metadata': self.agent_metadata[name]} for name in self.active_agents}
...
class DynamicNodeSupervisor(ReactAgent):
    ...
    def _update_choice_model(self):
        ...
        if active_agents:
            ...
            self.agent_choice_model = DynamicChoiceModel.from_choices(choices, name='AgentRouting', description="Select which agent to route the task to, or 'none' to handle it yourself")
    ...
    def _setup_supervisor_tools(self):
        ...
        def analyze_task_requirements(task: str) -> dict[str, Any]:
            ...
            if any((word in task_lower for word in ['calculate', 'compute', 'math', 'sum', 'multiply'])):
            ...
            return {'task': task, 'required_capabilities': capabilities, 'complexity': 'simple' if len(capabilities) <= 1 else 'complex'}
        ...
        def check_agent_availability(capability: str) -> dict[str, Any]:
            ...
            inactive_matches = [a for a in matching_agents if a not in self.agent_registry.active_agents]
            return {'capability': capability, 'active_agents': active_matches, 'inactive_agents': inactive_matches, 'total_matches': len(matching_agents)}
        ...
        def list_agents() -> dict[str, Any]:
            ...
            for name in self.agent_registry.agents:
                ...
                all_agents[name] = {'active': name in self.agent_registry.active_agents, 'description': metadata['description'], 'capabilities': metadata['capabilities'], 'activation_count': metadata['activation_count']}
            return all_agents
        if hasattr(self, 'engine') and self.engine:
            self.engine.tools.extend([analyze_task_requirements, check_agent_availability, activate_agent, route_to_agent, list_agents])
    ...
    def build_graph(self) -> BaseGraph:
        ...
        graph.add_conditional_edges('supervisor', self._route_decision, {'execute_agent': 'agent_execution', 'end': END})
    ...
    async def _agent_execution_node(self, state: DynamicSupervisorState) -> dict[str, Any]:
        ...
        try:
            ...
            state.execution_history.append({'timestamp': datetime.now().isoformat(), 'agent': state.agent_route, 'task': state.current_task, 'response_length': len(str(result)), 'success': True})
        except Exception as e:
            state.agent_response = f'Error executing {state.agent_route}: {e!s}'
            state.execution_history.append({'timestamp': datetime.now().isoformat(), 'agent': state.agent_route, 'task': state.current_task, 'error': str(e), 'success': False})
    ...
    async def _process_result_node(self, state: DynamicSupervisorState) -> dict[str, Any]:
        ...
        if state.agent_response:
            state.messages.append({'role': 'assistant', 'content': state.agent_response, 'metadata': {'source': 'agent_execution', 'timestamp': datetime.now().isoformat()}})
        return {'state': state}
...
async def test_dynamic_node_supervisor():
    ...
    research_engine = AugLLMConfig(name='research_engine', model='gpt-4', tools=[tavily_search_tool], system_message='You are a research specialist.').create()
    ...
    math_engine = AugLLMConfig(name='math_engine', model='gpt-4', tools=[calculate], system_message='You are a math specialist.').create()
    ...
    class Essay(BaseModel):
        ...
        body: list[str]
        conclusion: str
    essay_engine = create_structured_output_engine(model_config=LLMConfig(model='gpt-4'), output_schema=Essay, system_message='You are an essay writer.')
    essay_agent = SimpleAgent(name='essay_agent', engine=essay_engine)
    supervisor_engine = AugLLMConfig(name='supervisor_engine', model='gpt-4', tools=[], system_message='You are a dynamic task supervisor.').create()
    supervisor = DynamicNodeSupervisor(name='dynamic_supervisor', engine=supervisor_engine, state_schema=DynamicSupervisorState)
    supervisor.agent_registry.register('research_agent', research_agent, 'Web research and information gathering', ['research', 'search', 'information'], active=True)
    supervisor.agent_registry.register('math_agent', math_agent, 'Mathematical calculations and analysis', ['math', 'calculation', 'analysis'], active=True)
    supervisor.agent_registry.register('essay_agent', essay_agent, 'Essay writing and document composition', ['writing', 'essay', 'documentation'], active=False)
