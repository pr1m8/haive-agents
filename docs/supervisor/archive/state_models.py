...
class SerializedAgent(BaseModel):
    ...
    def from_agent(cls, agent: Any, metadata: AgentMetadata) -> 'SerializedAgent':
        ...
        if hasattr(agent, 'config'):
            try:
                config_json = json.dumps(agent.config.dict() if hasattr(agent.config, 'dict') else {})
            except:
                config_json = str(agent.config)
        return cls(metadata=metadata, agent_class=agent_class, agent_module=agent_module, serialized_data=serialized_data, config_json=config_json)
...
class SupervisorState(StateSchema):
    ...
    agents: dict[str, SerializedAgent] = Field(default_factory=dict, description='Registry of available agents')
    tools: dict[str, BaseTool] = Field(default_factory=dict, description='Available tools mapped by name')
    tool_mappings: dict[str, ToolMapping] = Field(default_factory=dict, description='Metadata about each tool')
    execution_context: ExecutionContext = Field(default_factory=ExecutionContext, description='Current execution state')
    last_result: Any | None = None
    execution_history: list[dict[str, Any]] = Field(default_factory=list)
    auto_sync_tools: bool = Field(default=True, description='Whether to automatically sync tools with agents')
    max_history_size: int = Field(default=100, description='Maximum execution history entries to keep')
    ...
    def sync_tools_if_enabled(self) -> 'SupervisorState':
        ...
        for agent_name in self.agents:
            ...
            if tool_name not in self.tool_mappings:
                self.tool_mappings[tool_name] = ToolMapping(tool_name=tool_name, agent_name=agent_name, category='handoff', description=f'Hand off task to {agent_name}', is_dynamic=True)
        mappings_to_remove = []
...
class DynamicSupervisorState(SupervisorState):
    ...
    agent_templates: dict[str, dict[str, Any]] = Field(default_factory=dict, description='Templates for creating new agents')
    can_create_agents: bool = Field(default=True, description='Whether supervisor can create new agents')
    creation_history: list[dict[str, Any]] = Field(default_factory=list, description='History of agent creations')
    ...
    def enforce_agent_limit(self) -> 'DynamicSupervisorState':
        ...
        if len(self.agents) > self.max_agents:
            agents_by_usage = sorted(self.agents.items(), key=lambda x: x[1].metadata.last_used or datetime.min, reverse=True)
    ...
    def record_agent_creation(self, agent_name: str, template_used: str | None=None) -> None:
        ...
        self.creation_history.append({'agent_name': agent_name, 'template_used': template_used, 'created_at': datetime.utcnow().isoformat(), 'total_agents': len(self.agents)})
    model_config = ConfigDict(arbitrary_types_allowed=True)