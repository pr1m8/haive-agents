...
class DynamicActivationSupervisor(ReactAgent):
    ...
    def _update_available_tools(self):
        ...
        if hasattr(self, 'engine') and self.engine and hasattr(self.engine, 'tools'):
            self.engine.tools = [tool for tool in self.engine.tools if not (tool.name.startswith('transfer_to_') or tool.name.startswith('forward_to_') or tool.name == 'forward_message')]
        active_agents = self.agent_registry.get_active_agents()
        handoff_tools = []
        for agent_name, agent_info in active_agents.items():
            transfer_tool = create_handoff_tool(target_agent_name=agent_name, target_agent=agent_info['agent'], description=f'Transfer control to {agent_name}: {agent_info['description']}')
            handoff_tools.append(transfer_tool)
            forward_tool = create_forward_message_tool(target_agent_name=agent_name, description=f'Forward a message to {agent_name} for processing')
        ...
        def activate_dormant_agent(capability: str) -> str:
            ...
            capability_map = {'research': 'research_agent', 'math': 'math_agent', 'essay_writing': 'essay_writer_agent'}
        ...
        if hasattr(self, 'engine') and self.engine:
            ...
            if active_choices:
                self.capability_model = DynamicChoiceModel.from_choices(active_choices, name='active_agents')

async def test_dynamic_activation():
    """Test supervisor activating dormant agents as needed."""
    research_engine = AugLLMConfig(name='research_engine', model='gpt-4', tools=[tavily_search_tool], system_message='You are a research specialist. Use tavily search to find information.').create()
    research_agent = ReactAgent(name='research_agent', engine=research_engine)
    math_engine = AugLLMConfig(name='math_engine', model='gpt-4', tools=[add, multiply, calculate_compound_interest], system_message='You are a math specialist. Perform calculations as requested.').create()
    ...
    class Essay(BaseModel):
        ...
        conclusion: str
        word_count: int
    essay_engine = create_structured_output_engine(model_config=LLMConfig(model='gpt-4'), output_schema=Essay, system_message='You are an essay writing specialist. Create well-structured essays.')
    essay_writer_agent = SimpleAgent(name='essay_writer_agent', engine=essay_engine, config=SimpleAgentConfig(system_message='Essay writer with structured output'))
    supervisor_engine = AugLLMConfig(name='supervisor_engine', model='gpt-4', tools=[], system_message='You are a dynamic supervisor that manages specialized agents.\n\nYour workflow:\n1. Analyze the task to identify required capabilities\n2. Check which agents are currently active\n3. If a required capability is missing, activate the appropriate agent\n4. Route tasks to the appropriate specialized agents\n5. Coordinate their responses to complete the overall task\n\nImportant: Always check required capabilities before attempting to route tasks.').create()
    supervisor = DynamicActivationSupervisor(name='dynamic_supervisor', engine=supervisor_engine, state_schema=SupervisorState)
    supervisor.agent_registry.register('research_agent', research_agent, 'Research specialist with web search', active=True)
    supervisor.agent_registry.register('math_agent', math_agent, 'Math specialist with calculation tools', active=True)
    supervisor.agent_registry.register('essay_writer_agent', essay_writer_agent, 'Essay writer with structured output', active=False)
    supervisor._update_available_tools()
    await supervisor.arun('Calculate the compound interest on $10,000 at 5% for 10 years')
    await supervisor.arun('Research the benefits of renewable energy and write a short essay about it')
