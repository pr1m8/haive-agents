...
def test_multi_agent_state():
    ...
    state.update_agent_state("plannef", {"plan": "Step 1", "status": "planning"})
    state.update_agent_state("executof", {"result": None, "status": "waiting"})
    ...
    state.record_agent_output("plannef", {"plan": "Complete plan", "steps": 3})
    state.record_agent_output("executof", {"result": "Success", "data": [1, 2, 3]})
