...
def create_test_agents():
    ...
    # Simple LLM config
    config = AugLLMConfig(
        temperature=0.3, max_tokens=200, system_message="You are a helpful assistant."
    )

    # Create three agents for testing
    analyzer = SimpleAgentV3(
        name="analyzer",
        engine=config.model_copy(
            update={"system_message": "You analyze problems and identify key issues."}
        ),
        debug=True,
    )
    ...
    planner = SimpleAgentV3(
        name="planner",
        engine=config.model_copy(
            update={"system_message": "You create actionable plans based on analysis."}
        ),
        debug=True,
    )
    ...
    executor = SimpleAgentV3(
        name="executor",
        engine=config.model_copy(
            update={"system_message": "You execute plans and report results."}
        ),
...
def test_v4_sequential():
    ...
    execution_mode="sequential"
        ...
        multi = EnhancedMultiAgentV4(
            name="v4_workflow", agents=[analyzer, planner, executor], execution_mode="sequential"
        )
        ...
        test_input = {
            "messages": [
                HumanMessage(
                    content="Analyze how to improve team productivity, create a plan, and execute it."
                )
            ]
        }
...
multi.add_conditional_edge(
    ...
    false_agent="planner"    # Normal flow
        ...
        result1 = compiled.invoke(
            {
                "messages": [
                    HumanMessage(content="Analyze our sales strategy and create improvement plan.")
                ]
            }
        )
        ...
        result2 = compiled.invoke(
            {
                "messages": [
                    HumanMessage(
                        content="URGENT: Analyze and fix the production issue immediately!"
                    )
                ]
            }
        )
...
def test_v3_if_available():
    ...
    adaptation_rate=0.2
        ...
        v3_kwargs = {"name": "v3_workflow", "agents": agents_dict, "execution_mode": "sequential"}
