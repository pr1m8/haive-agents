...
try:
    ...
    haive.core.schema.prebuilt.enhanced_multi_agent_state = haive.core.schema.prebuilt.multi_agent_state
...
class TestEnhancedMultiAgentComparison:
    ...
    def test_2_conditional_routing_comparison(self):
        ...
        try:
            ...
            v3_metrics.features_used = ["advanced_routing", "performance_mode", "conditional_routing"]
            v3_metrics.setup_time = time.time() - start
        ...
        try:
            ...
            v4_multi.add_conditional_edge(
                "analyzer",
                lambda state: "formatter" if "format" in str(state.get("messages", [])[-1].content).lower() else "processor",
    ...
    def test_4_state_projection_comparison(self):
        ...
        test_input = {
            "messages": [HumanMessage(
                content="Analyze market trends and process the data with metrics."
            )]
        }
        ...
        try:
            ...
            if isinstance(v3_result, dict):
                ...
                has_structured = any(key in v3_result for key in ["category", "confidence", "status", "items_processed"])
        ...
        try:
            ...
            if isinstance(v4_result, dict):
                ...
                has_structured = any(key in v4_result for key in ["category", "confidence", "status", "items_processed"])
    ...
    def test_5_developer_experience_comparison(self):
        ...
        try:
            ...
            v3_multi.add_conditional_routing(
                "analyzer", 
                lambda state: "processor" if "data" in str(state.get("messages", [])[-1].content) else "formatter",
            ...
            v3_metrics.features_used = [
                "dict_agents", "branch_mode", "performance_mode", "debug_mode",
                "advanced_routing", "conditional_routing", "parallel_groups"
            ]
            v3_metrics.success = True
        ...
        try:
            ...
            v4_metrics.features_used = [
                "list_agents", "manual_mode", "add_edge", "add_conditional_edge",
                "clean_api"
            ]
            v4_metrics.success = True
    ...
    def _report_comparison(self, test_name: str, v3_metrics: ComparisonMetrics, v4_metrics: ComparisonMetrics):
