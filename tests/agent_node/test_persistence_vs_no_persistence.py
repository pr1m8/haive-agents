...
def test_agent_direct_execution():
    ...
    print(f"📝 Query: {query}")
    print(f"🎯 Expected: SimpleTestModel(message='Hello World', count=42)")
    ...
    try:
        ...
        print(f"   Has get_latest_structured_output: {hasattr(result_run, 'get_latest_structured_output')}")
        ...
        if hasattr(result_run, 'get_latest_structured_output'):
            ...
            if isinstance(structured, SimpleTestModel):
                print(f"   ✅ SUCCESS: Got SimpleTestModel directly")
                print(f"      message: {structured.message}")
                print(f"      count: {structured.count}")
            else:
                print(f"   ❌ WRONG TYPE: Expected SimpleTestModel, got {type(structured)}")
        else:
            print(f"   ❌ NO METHOD: Missing get_latest_structured_output()")
    ...
    try:
        ...
        print(f"✅ agent.invoke() completed")
                    ...
                    for i, msg in enumerate(value):
                        if hasattr(msg, 'tool_calls') and msg.tool_calls:
                            ...
                            for j, tool_call in enumerate(msg.tool_calls):
                                print(f"       Tool call {j}: {tool_call.name} = {tool_call.args}")
                        else:
...
def test_agent_node_v3_without_persistence():
    ...
    print(f"✅ Created agent WITHOUT persistence")
    ...
    state = MultiAgentState(
        ...
        messages=[{"role": "user", "content": "Respond with message 'No Persistence' and count 100"}]
    )
    ...
    try:
        result = node(state)
        print(f"✅ AgentNodeV3 execution completed")
...
def test_agent_node_v3_with_persistence():
    ...
    print(f"✅ Created agent WITH persistence")
    print(f"   Agent persistence config: {getattr(agent, 'persistence', 'None')}")
    print(f"   Agent state persistence: {getattr(agent, 'state_persistence', 'None')}")
    ...
    state = MultiAgentState(
        ...
        messages=[{"role": "user", "content": "Respond with message 'With Persistence' and count 200"}]
    )
...
def test_compare_agent_invoke_vs_node():
    ...
    if has_output_schema:
        if isinstance(raw_result, BaseModel):
            print(f"   ✅ Result is BaseModel - would use result.model_dump()")
            state_update = raw_result.model_dump()
        elif isinstance(raw_result, dict):
            print(f"   ⚠️  Result is dict - would use result directly")
            state_update = raw_result
        else:
            print(f"   ❌ Result is other - would wrap in fallback")
        ...
        if 'message' in state_update and 'count' in state_update:
            print(f"   ✅ SUCCESS: Found direct structured fields!")
            print(f"      message: {state_update['message']}")
            print(f"      count: {state_update['count']}")
        else:
            print(f"   ⚠️  No direct structured fields found")
