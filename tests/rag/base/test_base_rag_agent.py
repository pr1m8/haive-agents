...
def sample_documents():
    ...
    documents = [
        ...
        Document(page_content="It provides tools and abstractions for working with LLMs and other components.",
                 metadata={"source": "docs", "section": "intro"}),
        Document(page_content="LangGraph is part of the LangChain ecosystem for building stateful agent workflows.",
                 metadata={"source": "docs", "section": "langgraph"}),
        Document(page_content="Agents are systems that use LLMs to determine which actions to take.",
...
def direct_vs_rag_agent(vectorstore_config):
    ...
    console.print(Panel.fit("Creating direct vectorstore RAG agent", title="Setup", border_style="blue"))
    
    agent = BaseRAGAgent(
        BaseRAGConfig(
            name="direct_vectorstore_rag",
            retriever_config=vectorstore_config
        )
    )
...
def explicit_retriever_rag_agent(retriever_config):
    ...
    console.print(Panel.fit("Creating explicit retriever RAG agent", title="Setup", border_style="blue"))
...
def test_retriever_direct_invocation(retriever_config):
    ...
    console.print(Panel.fit("Testing direct retriever invocation", title="Test", border_style="green"))
...
def test_direct_vs_rag_agent(direct_vs_rag_agent):
    ...
    console.print(Panel.fit("Testing direct vectorstore RAG agent", title="Test", border_style="green"))
...
def test_explicit_retriever_rag_agent(explicit_retriever_rag_agent):
    ...
    console.print(Panel.fit("Testing explicit retriever RAG agent", title="Test", border_style="green"))
...
def test_alternative_mapping(retriever_config):
    ...
    console.print("Mapping correctly applied state.question → query → documents → state.search_results")
