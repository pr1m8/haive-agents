...
class TestSolutionScorer:
    ...
    async def test_score_math_solutions(self):
        ...
        for scored in scoring.scored_solutions:
            if "incorrect" in scored.solution or "= 6" in scored.solution:
                assert scored.score < 0.5, f"Incorrect solution should have low score: {scored.solution}"
            elif "6 + 6 + 6 + 6" in scored.solution:
                # This uses 4 numbers instead of 3
                assert scored.score < 0.8, f"Solution with wrong number count should not be perfect: {scored.solution}"
            else:
                # Correct solutions
                assert scored.score > 0.7, f"Correct solution should have high score: {scored.solution}"
    ...
    async def test_get_best_solutions(self):
        ...
        best_solutions = await scorer.get_best_solutions(
            problem, 
            candidates, 
            top_k=3
        )
    ...
    async def test_score_partial_solutions(self):
        ...
        for scored in scoring.scored_solutions:
            ...
            elif "if n == 0" in scored.solution and "return 1" in scored.solution and "factorial(n-1)" not in scored.solution:
                # Partial solution
...
if __name__ == "__main__":
    async def run_tests():
        ...
        print("Testing math solution scoring...")
        await test.test_score_math_solutions()
        print("✓ Math scoring test passed")
        
        print("\nTesting Game of 24 scoring...")
        await test.test_score_game_of_24_solutions()
        print("✓ Game of 24 scoring test passed")
        
        print("\nTesting top-k best solutions...")
        await test.test_get_best_solutions()
        print("✓ Best solutions test passed")
        
        print("\nTesting partial solution scoring...")
        await test.test_score_partial_solutions()
        print("✓ Partial solutions test passed")
