...
def debug_format_instructions_conditions():
    ...
    print(f"1. CHECKING INDIVIDUAL CONDITIONS:")
    ...
    if not include_fi:
        print(f"   ❌ FAIL: include_format_instructions is False")
        return "include_format_instructions"
    else:
        print(f"   ✅ PASS: include_format_instructions is True")
    ...
    if has_format_instructions:
        print(f"   ❌ FAIL: format_instructions already exists")
        return "already_exists"
    else:
        print(f"   ✅ PASS: format_instructions not in partial_variables")
    ...
    if not has_model:
        print(f"   ❌ FAIL: structured_output_model is None")
        return "no_model"
    else:
        print(f"   ✅ PASS: structured_output_model exists")
    ...
    if hasattr(config, '_should_setup_format_instructions'):
        ...
        if should_setup:
            print(f"   ✅ Method returns True as expected")
            return "success"
        else:
            print(f"   ❌ Method returns False - THERE'S A HIDDEN CONDITION!")
        ...
        print(f"   ❌ _should_setup_format_instructions method not found!")
...
def test_manual_format_instructions_setup():
    ...
    print(f"\n" + "=" * 60)
    print(f"🛠️  TESTING MANUAL FORMAT INSTRUCTIONS SETUP")
    print("=" * 60)
    ...
    if hasattr(config, '_setup_format_instructions'):
        ...
        try:
            ...
            print(f"   ✅ Manual setup completed without error")
...
def test_direct_partial_variables_modification():
    ...
    print(f"\n" + "=" * 60)
    print(f"🔧 TESTING DIRECT PARTIAL VARIABLES MODIFICATION")
    print("=" * 60)
    ...
    if hasattr(config, '_format_instructions_text') and config._format_instructions_text:
        ...
        try:
            ...
            if has_instructions:
                print(f"   ✅ Direct modification successful!")
                return "direct_success"
            else:
                print(f"   ❌ Direct modification failed")
        ...
        print(f"1. ❌ No internal format instructions found")
