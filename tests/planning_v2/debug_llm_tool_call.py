...
def debug_llm_prompt_and_tools():
    ...
    try:
                        ...
                        if 'steps' in json_schema.get('properties', {}):
                            ...
                            if 'items' in steps_def and '$ref' in steps_def['items']:
                                ...
                                if '$defs' in json_schema:
                                    ...
                                    print(f"     ❌ No $defs section in schema!")
        ...
        if hasattr(result, 'tool_calls') and result.tool_calls:
            ...
            if 'steps' in args:
                steps = args['steps']
                print(f"   Steps returned by LLM:")
                for i, step in enumerate(steps):
                    print(f"     Step {i}: {step} (type: {type(step)})")
                    if isinstance(step, str):
                        print(f"       ❌ LLM returned string instead of object!")
                    elif isinstance(step, dict) and 'description' in step:
                        print(f"       ✅ LLM returned proper object structure")
...
Do NOT return plain strings in the steps array. Each step must be a proper Task object."""
    ...
    print(f"1. Testing with explicit guidance...")
    ...
    if hasattr(result, 'tool_calls') and result.tool_calls:
        ...
        print(f"2. Result with explicit guidance:")
        if 'steps' in args:
            steps = args['steps']
            for i, step in enumerate(steps):
                print(f"   Step {i}: {step} (type: {type(step)})")
                if isinstance(step, dict) and 'description' in step:
                    ...
                    print(f"     ❌ Still incorrect structure!")
        
        return args
    else:
        print(f"2. ❌ No tool calls in result")
        return None
