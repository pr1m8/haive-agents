            ...
            if hasattr(msg_template, 'prompt'):
                if hasattr(msg_template.prompt, 'template'):
                    ...
                    if '{format_instructions}' in template_text:
                        print(f"     ✅ Found {{format_instructions}} placeholder!")
                    else:
                        print(f"     ❌ No {{format_instructions}} placeholder")
    ...
    try:
        ...
        for i, msg in enumerate(formatted.messages):
            ...
            if 'JSON schema' in content or 'format' in content.lower():
                print(f"     ✅ Format instructions appear to be in content!")
            else:
                print(f"     ❌ No format instructions in content")
...
def debug_structured_output_setup():
    ...
    for version in ["v1", "v2"]:
        ...
        if version == "v1":
            print(f"   V1 should rely on format instructions in prompt")
        else:
            print(f"   V2 should rely on tool schema, but may also use format instructions")
