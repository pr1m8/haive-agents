...
try:
    from haive.agents.document_modifiers.kg.kg_base.models import (
        GraphTransformer as _GraphTransformer,
    )
    from haive.agents.document_modifiers.kg.kg_map_merge.models import (
...
Focus on relationships that are explicitly mentioned or strongly implied."""
    ...
    def build_graph(self) -> BaseGraph:
        ...
        graph.add_conditional_edges("pre_hook", self.route_from_pre_hook, routing_map)  # type: ignore
        ...
        try:
            ...
            if route in [
                ...
                route_data["memory_consolidation_prep"] = {
                    ...
                    "old_memories": len(
                        [m for m in state.current_memories if m.memory_type != "meta"]
                    ),
    ...
    def process_memory_node(self, state: MemoryStateWithTokens) -> Command:
        ...
        try:
            ...
            operation_result: dict[str, Any] = {"operation_timestamp": datetime.now().isoformat()}
            ...
            elif any(word in content.lower() for word in ["search", "find", "look fof"]):
    ...
    def emergency_compress_node(self, state: MemoryStateWithTokens) -> Command:
        ...
        for memory in state.current_memories:
            if (
                ...
                or "summary"
                in (memory.metadata.get("tags", []) if hasattr(memory, "metadata") else [])
            ):
    ...
    def update_summary_node(self, state: MemoryStateWithTokens) -> Command:
        ...
        if hasattr(state, "last_operation") and state.last_operation:
            recent_activity += f" Last operation: {
                state.last_operation.get('type', 'unknown')
                if isinstance(state.last_operation, dict)
                else 'unknown'
            }"
    ...
    def transform_to_graph_node(self, state: MemoryStateWithTokens) -> Command:
        ...
        try:
            ...
            for memory in state.current_memories:
                content_parts.append(f"[Memory:{memory.memory_type}] {memory.content}")
    ...
    def summarize_memories_node(self, state: MemoryState) -> dict[str, Any]:
        ...
        try:
            ...
            memories_text = "\n\n".join([f"[{m.memory_type}] {m.content}" for m in to_summarize])
            ...
            for memory in state.current_memories:
                ...
                if self.engine and hasattr(self.engine, "memory_rewrite_prompt"):
                    ...
                    rewritten_memory = EnhancedMemoryItem(
                        ...
                        else MemoryType(memory.memory_type),
