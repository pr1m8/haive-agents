...
class LLMRAGAgent(BaseRAGAgent):
    ...
    def setup_workflow(self) -> None:
        ...
        logger.info(f"Setting up LLM RAG workflow with components:")
        logger.info(f"- Retriever: {retriever_engine.name}")
        logger.info(f"- LLM: {llm_engine.name}")
        logger.info(f"- Relevance Checker: {relevance_checker.name if relevance_checker else 'None'}")
        ...
        def generate_answer(state):
            ...
            try:
                ...
                if not state.is_relevant:
                    return Command(
                        update={"answer": "The retrieved documents are not relevant to the question."},
                        goto=END
                    )
                ...
                return Command(
                    update={"answer": f"Error generating answer: {str(e)}", "error": str(e)},
                    goto=END
                )
