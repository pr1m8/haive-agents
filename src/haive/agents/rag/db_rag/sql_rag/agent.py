from haive.agents.rag.db_rag.sql_rag.state import OverallState, InputState, OutputState
...
from haive.agents.rag.db_rag.sql_rag.utils import (
    create_sql_toolkit, create_tool_node_with_fallback, 
    get_all_toolkit_tools, explore_database_schema
)
...
class SQLRAGAgent(Agent[SQLRAGConfig]):
    ...
    def generate_query(self, state: OverallState) -> Command:
        ...
        try:
            ...
            if ("what tables" in question_lower or "list tables" in question_lower or 
                "show tables" in question_lower or "which tables" in question_lower):
        ...
        try:
            ...
            if self.config.hallucination_check and "hallucination_check" in self.engines:
                try:
                    ...
                    # If hallucinations detected, provide a warning
                    if hasattr(hallucination_result, 'hallucination_detected') and hallucination_result.hallucination_detected:
                        warning = f"\n\nWarning: The answer may contain information not supported by the data. Areas of concern: {hallucination_result.problem_areas}"
                        result["answer"] = answer + warning
    ...
    def setup_workflow(self) -> None:
        ...
        self.graph.add_conditional_edges("check_domain_relevance", lambda state: domain_branch.evaluate(state), lambda x: x["next_action"])
        ...
        self.graph.add_conditional_edges("validate_query", lambda state: validation_branch.evaluate(state), lambda x: x["next_action"])
