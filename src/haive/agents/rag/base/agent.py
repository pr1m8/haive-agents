...
class BaseRAGAgent(Agent[BaseRAGConfig]):
    ...
    def setup_workflow(self) -> None:
        ...
        logger.info(
            f"Setting up retriever node with engine: {retriever_engine.name} (ID: {getattr(retriever_engine, 'id', 'unknown')})"
        )
        ...
        }  # Map retriever result.documents to state.retrieved_documents
        ...
        try:
            ...
            if isinstance(test_result, dict):
                ...
                if "documents" in test_result:
                    logger.info(
                        f"Found {len(test_result['documents'])} documents in 'documents' key"
                    )
        ...
        def retriever_node(state):
            ...
            try:
                ...
                logger.info(
                    f"Retriever returned result of type: {type(result).__name__}"
                )
                ...
                if isinstance(result, list):
                    ...
                    logger.info(
                        f"Retrieved {len(documents)} documents from 'documents' key"
                    )
                    ...
                    logger.warning(
                        f"Unexpected retriever result format: {type(result).__name__}"
                    )
                    return Command(
                        update={
                            "retrieved_documents": [],
        ...
        retriever_node_config = NodeConfig(
        ...
        graph_builder.add_node("retrieve_documents", retriever_node_config)
        ...
        logger.info(
            f"State schema fields: {list(self.config.state_schema.__annotations__.keys())}"
        )
