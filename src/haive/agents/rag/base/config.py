...
class BaseRAGConfig(AgentConfig):
    ...
    retriever_config: BaseRetrieverConfig | VectorStoreConfig = Field(..., description='Configuration for the retriever component')
    ...
    def convert_vector_store_to_retriever(cls, data: dict[str, Any]) -> dict[str, Any]:
        ...
        if isinstance(data, dict) and 'retriever_config' in data:
            if isinstance(data['retriever_config'], VectorStoreConfig):
                ...
                data['retriever_config'] = VectorStoreRetrieverConfig(name=f'retriever_for_{data.get('name', 'rag_agent')}', vector_store_config=vs_config)
        return data
