...
class DocumentGrade(BaseModel):
    ...
    binary_score: Literal['yes', 'no'] = Field(description="Relevance score: 'yes' if relevant, 'no' if not relevant")
...
class AgenticRAGAgent[TInput: BaseModel, TOutput: BaseModel](ReactAgent[TInput, TOutput], ToolRouteMixin):
    ...
    - Generic type safety
    """
    retriever_engine: BaseRetrieverConfig = Field(..., description='Retrieval engine for document search')
    grader_engine: AugLLMConfig | None = Field(default=None, description='Engine for grading document relevance')
    rewriter_engine: AugLLMConfig | None = Field(default=None, description='Engine for rewriting queries')
    grade_documents_threshold: float = Field(default=0.7, description='Threshold for document relevance grading')
    max_retrieval_attempts: int = Field(default=3, description='Maximum attempts to find relevant documents')
    enable_query_rewriting: bool = Field(default=True, description='Whether to rewrite queries for better retrieval')
    ...
    def setup_agentic_rag(self) -> 'AgenticRAGAgent':
        ...
        if not self.grader_engine:
            self.grader_engine = AugLLMConfig(llm_config=self.llm_engine.llm_config, prompt_template=self._create_grading_prompt(), structured_output_model=DocumentGrade, structured_output_version='v1')
        self.engines['grader'] = self.grader_engine
        if not self.rewriter_engine:
            self.rewriter_engine = AugLLMConfig(llm_config=self.llm_engine.llm_config, prompt_template=self._create_rewriting_prompt(), structured_output_model=QueryRewrite, structured_output_version='v1')
    ...
    def _create_retrieval_tool(self) -> BaseTool:
        ...
        return StructuredTool.from_function(func=retrieve_documents, name='retrieve_documents', description='Search and retrieve relevant documents for answering questions')
    ...
    def _create_grading_tool(self) -> BaseTool:
        ...
        return StructuredTool.from_function(func=grade_document_relevance, name='grade_documents', description='Grade whether retrieved documents are relevant to the user question')
    ...
    def _create_rewriting_tool(self) -> BaseTool:
        ...
        def rewrite_query(original_query: str, feedback: str='') -> str:
            ...
            try:
                result = self.rewriter_engine.invoke({'original_query': original_query, 'feedback': feedback})
        ...
        return StructuredTool.from_function(func=rewrite_query, name='rewrite_query', description='Rewrite the user query to improve retrieval results')
    ...
    def _create_answer_generation_tool(self) -> BaseTool:
        ...
        def generate_answer_from_context(question: str, context: str) -> str:
            ...
            answer_prompt = ChatPromptTemplate.from_template("You are an assistant for question-answering tasks. Use the following retrieved context to answer the question. If you don't know the answer, say you don't know. Keep the answer concise and cite the context.\\n\\nQuestion: {question}\\n\\nContext: {context}\\n\\nAnswer:")
            try:
        ...
        return StructuredTool.from_function(func=generate_answer_from_context, name='generate_answer', description='Generate final answer using retrieved context')
    ...
    @classmethod
    def from_documents(cls, documents: list[Document], llm_config: LLMConfig, embedding_config: Any | None=None, **kwargs) -> 'AgenticRAGAgent':
        ...
        """
        retriever_engine = BaseRetrieverConfig.from_documents(documents=documents, embedding_config=embedding_config, name='Agentic RAG Retriever')
        llm_engine = AugLLMConfig(llm_config=llm_config, prompt_template=cls._create_agentic_prompt(), name='Agentic RAG LLM')
        return cls(llm_engine=llm_engine, retriever_engine=retriever_engine, name=kwargs.get('name', 'Agentic RAG Agent'), **kwargs)

    @staticmethod
    def _create_agentic_prompt() -> ChatPromptTemplate:
        """Create agentic reasoning prompt for deciding when to retrieve."""
        return ChatPromptTemplate.from_messages([('system', "You are an intelligent assistant that can decide when to retrieve information.\n\nYou have access to these tools:\n- retrieve_documents: Search for relevant documents\n- grade_documents: Check if retrieved documents are relevant\n- rewrite_query: Improve queries for better retrieval\n- generate_answer: Create final answers from context\n\nFor each user question, decide whether to:\n1. Retrieve documents if you need more information\n2. Respond directly if you can answer without retrieval\n3. Grade documents if you've retrieved them\n4. Rewrite queries if documents aren't relevant\n5. Generate final answers when you have good context\n\nThink step by step about what action to take."), ('human', '{query}')])

    @staticmethod
    def _create_grading_prompt() -> ChatPromptTemplate:
        """Create prompt for document relevance grading."""
        return ChatPromptTemplate.from_messages([('system', 'You are a document relevance grader. Assess whether retrieved documents are relevant to the user question.\n\nGrade documents as:\n- "yes" if they contain information that helps answer the question\n- "no" if they are not relevant or don\'t contain useful information\n\nProvide reasoning for your decision.'), ('human', 'Question: {question}\n\nRetrieved Context: {context}\n\nGrade the relevance of this context to the question.')])

    @staticmethod
    def _create_rewriting_prompt() -> ChatPromptTemplate:
        """Create prompt for query rewriting."""
        return ChatPromptTemplate.from_messages([('system', 'You are a query rewriting expert. Improve queries to get better retrieval results.\n\nConsider:\n- Making queries more specific\n- Adding relevant keywords\n- Clarifying ambiguous terms\n- Focusing on key concepts\n\nExplain what changes you made and why.'), ('human', 'Original query: {original_query}\n\nFeedback on previous results: {feedback}\n\nRewrite this query for better document retrieval.')])

def create_agentic_rag_agent(documents: list[Document], llm_config: LLMConfig, embedding_config: Any | None=None, **kwargs) -> AgenticRAGAgent:
    """Create agentic RAG agent with sensible defaults."""
    return AgenticRAGAgent.from_documents(documents=documents, llm_config=llm_config, embedding_config=embedding_config, **kwargs)

def create_memory_aware_agentic_rag(documents: list[Document], llm_config: LLMConfig, memory_config: Any | None=None, **kwargs) -> AgenticRAGAgent:
