...
class ReWOOState(BaseModel):
    ...
    plan: RewooPlan | None = None  # The plan with evidence references
    results: dict[str, dict[str, str]] = Field(
        default_factory=dict,
        description="Evidence collected, keyed by evidence_ref -> {tool_name: result}"
    )
    current_step_index: int | None = None  # Index of current step in execution
    final_solution: list[dict[str, Any]] | None = None  # The final solution
