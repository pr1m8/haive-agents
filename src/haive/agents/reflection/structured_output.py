...
This module provides reflection agents that use structured output models combined with a
post-processing hook pattern for extracting results.
"""
...
class ReflectionLoop:
    ...
    async def iterate(self, query: str,
        ...
        return {
            ...
            "improved": (
                len(
                    quality_scores) > 0 and quality_scores[-1] > quality_scores[0]
                if quality_scores
                else False
            ),
        }
...
def create_reflection_agent(
    ...
    """
    return StructuredReflectionAgent(
...
def create_improvement_agent(
    ...
    """
    return StructuredImprovementAgent(
    name=name, temperature=temperature, **kwargs)
    ...
    if reflection:
        ...
        if reflection.critique.needs_revision:
            ...
            if second_reflection:
                print(
                    f"New Quality Score: {
    second_reflection.critique.overall_quality:.2f}"
                )
                print(
                    f"Still Needs Revision: {
    second_reflection.critique.needs_revision}"
                )
...
async def example_iterative_reflection():
    ...
    ' → '.join(
        f'{
            q:.2f}' for q in result['quality_scores'])}"
    )
