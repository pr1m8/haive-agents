...
class SimpleAgentWithValidation(Agent):
    ...
    temperature: Optional[float] = Field(
        default=None, description="Temperature")
    ...
    force_tool_use: Optional[bool] = Field(
        default=None, description="Force tool use")
    ...
    system_message: Optional[str] = Field(
        default=None, description="System message")
    llm_config: Optional[LLMConfig] = Field(
        default=None, description="LLM config")
    ...
    def _sync_fields_to_engine(self) -> None:
        ...
        if self.temperature is not None and hasattr(
        ...
        if self.force_tool_use is not None and hasattr(
                self.engine, "force_tool_use"):
        ...
        if self.system_message is not None and hasattr(
    ...
    def _needs_parser_node(self) -> bool:
        ...
        pydantic_tools = [
            tool for tool,
            route in tool_routes.items() if route == "pydantic_model"]
    ...
    def _has_force_tool_use(self) -> bool:
        ...
        return bool(
            getattr(
                ...
                "force_tool_choice",
                False) or (
                self.force_tool_use is not None and self.force_tool_use) or (
                    self.structured_output_model is not None) or (
                        getattr(
                            self.engine,
    ...
    def create_runnable(self, runnable_config: Dict[str, Any] = None) -> Any:
        ...
        if hasattr(
                self,
                "graph") and self.graph and hasattr(
                self.graph,
                "metadata"):
            initial_values = {}
    ...
    def from_engine(
        ...
        """
        return cls(
            name=name or "Simple Agent with Validation",
            engine=engine,
            **kwargs)
    ...
    def create_with_tools(
        ...
        """
        return cls(
            name=name or "Tool Agent with Validation",
            tools=tools,
            **kwargs)

    @classmethod
    def create_strict_validation(
        cls, engine: AugLLMConfig, name: Optional[str] = None, **kwargs
    ):
