...
class DynamicMultiAgentSupervisor(MultiAgent):
    ...
    """
    enable_dynamic_management: bool = Field(default=True, description='Enable dynamic agent addition/removal')
    supervisor_engine: Any | None = Field(default=None, description='Engine for supervisor decision making')
    use_choice_model: bool = Field(default=True, description='Use DynamicChoiceModel for routing decisions')
    ...
    def _setup_hybrid_schema(self) -> None:
        ...
        composed_schema = AgentSchemaComposer.from_agents(agents=agent_list, name=f'{self.__class__.__name__}BaseState', separation=self.schema_separation, include_meta=self.include_meta)
        ...
        enhanced_composer.add_field('dynamic_agent_registry', Optional[dict[str, Any]], default_factory=dict, description='Dynamic agent registry for runtime management')
        enhanced_composer.add_field('supervisor_decisions', list[dict[str, Any]], default_factory=list, description='History of supervisor routing decisions')
        enhanced_composer.add_field('dynamic_coordination_active', bool, default=False, description='Whether dynamic coordination is active')
    ...
    def _create_dynamic_supervisor_node(self) -> Any:
        ...
        async def supervisor_node(state: Any, config=None) -> dict[str, Any]:
            ...
            if self._dynamic_supervisor:
                supervisor_state = {'messages': messages, 'registered_agents': getattr(state, 'dynamic_agent_registry', {}), 'supervisor_decisions': getattr(state, 'supervisor_decisions', [])}
                decision_result = await self._dynamic_supervisor.route_request_internal(supervisor_state)
                updates = {'supervisor_decisions': [*getattr(state, 'supervisor_decisions', []), decision_result], 'dynamic_coordination_active': True}
                return updates
    ...
    async def register_agent_dynamically(self, agent: Agent, capability: str | None=None) -> bool:
        ...
        if not self._dynamic_supervisor:
            ...
            return False
        try:
            success = await self._dynamic_supervisor.register_agent(agent, capability_description=capability or f'Dynamically added {agent.name}', rebuild_graph=True)
            if success:
    ...
    def get_dynamic_status(self) -> dict[str, Any]:
        ...
        status = {'dynamic_management_enabled': self.enable_dynamic_management, 'execution_mode': self.execution_mode, 'total_agents': len(self.agents), 'supervisor_available': self._dynamic_supervisor is not None}
        if self._dynamic_supervisor:
            status.update({'registered_agents': self._dynamic_supervisor.agent_registry.get_available_agents(), 'coordination_status': self._dynamic_supervisor.get_coordination_status()})
        return status
...
def create_compatible_supervisor(agents: Sequence[Agent], name: str='Compatible Supervisor', enable_dynamic: bool=True, supervisor_engine: Any=None) -> DynamicMultiAgentSupervisor | MultiAgent:
...
def migrate_from_multi_agent(multi_agent: MultiAgent) -> DynamicMultiAgentSupervisor:
    ...
    """
    return DynamicMultiAgentSupervisor(name=multi_agent.name, agents=multi_agent.agents, execution_mode=ExecutionMode.HIERARCHICAL, enable_dynamic_management=True, schema_separation=getattr(multi_agent, 'schema_separation', 'smart'), include_meta=getattr(multi_agent, 'include_meta', True))