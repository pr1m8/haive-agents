...
from haive.agents.complex_extraction.config import ComplexExtractionAgentConfig
from haive.agents.complex_extraction.models import PatchFunctionParameters, RetryStrategy
from haive.agents.complex_extraction.utils import decode, default_aggregator, encode
...
class ComplexExtractionAgent(Agent[ComplexExtractionAgentConfig]):
    ...
    def __init__(self, config: ComplexExtractionAgentConfig=ComplexExtractionAgentConfig()):
        ...
        self.llm = config.llm_config.instantiate_llm()
    ...
    def _bind_validator_with_retries(
        self,
        llm: Runnable[Sequence[AnyMessage], AIMessage] | Runnable[Sequence[BaseMessage], BaseMessage],
        *,
    ...
    #def compile(self):
        #self.app = dec
    def _prepare_extraction_messages(self, input_data: str | list[str] | dict[str, Any] | BaseModel) -> list[BaseMessage]:
...
def create_complex_extraction_agent(
    ...
    **kwargs
    ...
    llm_config = AzureLLMConfig(
        model=model,
        parameters={"temperature": 0.1}  # Lower temperature for extraction
    )
