    ...
    for msg in messages:
        ...
        if is_tool_message:
            ...
            if not tool_call_id and tool_name in tool_calls_map:
                logger.debug(f"Fixing message for tool '{tool_name}' with ID {tool_calls_map[tool_name]}")
                ...
                if not tool_call_id:
                    logger.warning(f"Can't fix tool message for '{tool_name}' - no matching tool call found")
...
def create_debug_tool_node(tools: list[Any]):
    ...
    def debug_tool_node(state: dict[str, Any]) -> dict[str, Any]:
        ...
        if isinstance(last_ai_message, AIMessage):
            ...
            elif hasattr(last_ai_message, "additional_kwargs") and "tool_calls" in last_ai_message.additional_kwargs:
        ...
        for tool_call in tool_calls:
            ...
            try:
                ...
                new_messages.append(tool_message)
                logger.debug(f"Created tool message: {tool_message.model_dump() if hasattr(tool_message, 'model_dump') else tool_message}")
