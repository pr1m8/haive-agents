...
class ReflectionResult(BaseModel):
    ...
    superfluous: str = Field(description="Analysis of what is superfluous in the response")
    score: int = Field(description="Score from 0-10 on the quality of the response", ge=0, le=10)
    found_solution: bool = Field(description="Whether this response fully solves the problem")
...
class ReflectionOutput(BaseModel):
    ...
    reflection: ReflectionResult = Field(description="The reflection on the original answer")
    search_queries: list[str] = Field(
        default_factory=list, description="Potential search queries for improving the answer"
    )
