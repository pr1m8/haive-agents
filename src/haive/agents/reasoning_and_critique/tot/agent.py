...
import logging
from typing import List, Dict, Any, Optional, Union, cast, Callable, TypeVar, Type, Generic
...
from haive.agents.reasoning_and_critique.tot.models import (
    Candidate, Score, ScoredCandidate, CandidateGeneration, CandidateEvaluation
)
...
class ToTAgent(Agent[TOTAgentConfig], Generic[T]):
    ...
    def setup_workflow(self) -> None:
        ...
        self.dynamic_graph.add_node(self.config.generator_node, self._generate_candidates)
        ...
        if self.parallel_evaluation:
            ...
            self.dynamic_graph.add_node("collect_evaluations", self._collect_evaluations)
        else:
            self.dynamic_graph.add_node(self.config.evaluator_node, self._score_candidates)
        ...
        if self.parallel_evaluation:
            ...
            self.dynamic_graph.add_edge("collect_evaluations", self.config.selector_node)
        else:
            ...
            self.dynamic_graph.add_edge(self.config.evaluator_node, self.config.selector_node)
    ...
    async def _generate_candidates(self, state: TOTState) -> Command:
        ...
        if hasattr(state, "current_seed") and state.current_seed:
            seed_content = ""
            if hasattr(state.current_seed, "candidate") and hasattr(state.current_seed.candidate, "content"):
            ...
            try:
                ...
                if hasattr(response, "to_score"):
                    ...
                    if score_match:
                        ...
                        score = Score(value=0.5, feedback="Could not extract score: " + text)
    ...
    def _select_best(self, state: TOTState) -> Command:
        ...
        sorted_candidates = sorted(
            scored_candidates, 
            key=lambda c: c.score.value if hasattr(c, "score") and hasattr(c.score, "value") else 0.0,
            reverse=True
        )
        ...
        if solved or state.depth >= self.max_depth:
            best_content = best_candidate.candidate.content if hasattr(best_candidate, "candidate") else str(best_candidate)
    ...
    def _should_expand_or_finish(self, state: TOTState) -> Union[str, END]:
        ...
        best_score = getattr(best_candidate, "value", 
