...
class StepResult(BaseModel):
    ...
    findings: str = Field(..., description="What was discovered or concluded in this step")
    evidence: list[str] = Field(
        default_factory=list, description="Supporting evidence or reasoning for the findings"
    )
    confidence: float = Field(
        ..., description="Confidence level in this step's results (0.0-1.0)", ge=0.0, le=1.0
    )
    next_step_recommendations: str | None = Field(
        default=None, description="Recommendations for subsequent steps based on these findings"
    )
...
class ExecutionResult(BaseModel):
    ...
    task_summary: str = Field(..., description="Brief summary of the task that was solved")
    step_results: list[StepResult] = Field(
        ..., description="Results from each executed reasoning step", min_length=1
    )
    final_solution: str = Field(..., description="The final solution or conclusion reached")
    solution_confidence: float = Field(
        ..., description="Overall confidence in the final solution (0.0-1.0)", ge=0.0, le=1.0
    )
    ...
    alternative_perspectives: list[str] = Field(
        default_factory=list, description="Alternative viewpoints or solutions considered"
    )
