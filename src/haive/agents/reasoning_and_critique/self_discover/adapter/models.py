...
class AdaptedModule(BaseModel):
    ...
    original_module: str = Field(..., description="The original module name/description")
    task_specific_adaptation: str = Field(
        ..., description="The module adapted specifically for this task"
    )
    concrete_steps: list[str] = Field(
        ..., description="Concrete steps or questions for applying this module", min_length=2
    )
...
class AdaptedModules(BaseModel):
    ...
    task_context: str = Field(..., description="Brief context about the task these are adapted for")
    adapted_modules: list[AdaptedModule] = Field(
        ..., description="List of adapted reasoning modules"
    )
    integration_strategy: str = Field(..., description="How these adapted modules work together")
