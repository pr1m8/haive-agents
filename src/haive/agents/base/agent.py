...
class Agent(
    ...
    def _setup_schemas(self) -> None:
        ...
        if (
            ...
            logger.debug(
                f"State schema already set by setup_agent() to {getattr(self.state_schema, '__name__', 'Unknown')}, skipping regeneration"
            )
            self._auto_derive_io_schemas()
            return
        engine_list = []
        ...
        try:
            if self.state_schema and self.use_prebuilt_base and engine_list:
                logger.debug(
                    f"Extending prebuilt schema {getattr(self.state_schema, '__name__', 'Unknown')} with engine fields"
                )
    ...
    def _auto_derive_io_schemas(self) -> None:
        ...
        if not self.output_schema:
                        ...
                        self.output_schema = create_model(
                            ...
                            **{
                                field_name: (
                                    structured_output,
                                    Field(
                                        description=f"Parsed {getattr(structured_output, '__name__', 'Unknown')}"
                                    ),
                                )
                            },
                        )
                        logger.debug(
                            f"Created output schema with structured field '{field_name}': {getattr(self.output_schema, '__name__', 'Unknown')}"
                        )
                    else:
                        self.output_schema = structured_output
                        logger.debug(
                            f"Using structured output model as output schema: {getattr(structured_output, '__name__', 'Unknown')}"
                        )
                    return
            ...
            if not self.output_schema:
                self.output_schema = create_model(
                    f"{self.name}Output", messages=(list[BaseMessage], Field(default_factory=list))
                )
    ...
    def get_schema_info(self) -> dict[str, Any]:
        ...
        info = {
            ...
            "state_schema": {
                "name": getattr(self.state_schema, "__name__", None) if self.state_schema else None,
                "class_engines": [],
            ...
            "input_schema": {
                "name": getattr(self.input_schema, "__name__", None) if self.input_schema else None,
                "fields": [],
            },
            "output_schema": {
                "name": getattr(self.output_schema, "__name__", None)
                if self.output_schema
                else None,
                "fields": [],
            },
    ...
    def display_schema_info(self) -> None:
        ...
        table.add_row(
            "  Supports Routing", "✅" if info["state_schema"]["supports_routing"] else "❌"
        )
