...
class SmartGenericEngineNode(GenericEngineNodeConfig[TInput, TOutput]):
    ...
    enable_smart_parsing: bool = Field(
        default=True, description="Enable smart output parsing post-processing"
    )
    ...
    custom_parsers: dict[str, BaseOutputParser] = Field(
        default_factory=dict, description="Custom parsers for specific content types"
    )

    post_processing_callables: list[Callable] = Field(
        default_factory=list, description="Custom post-processing functions"
    )
