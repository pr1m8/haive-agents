...
class ReactManyToolsConfig(ReactAgentConfig):
    ...
    max_tools_per_request: int = Field(
        default=10, description="Maximum number of tools to expose to the agent per request"
    )
    tool_categories: dict[str, list[str]] = Field(
        default_factory=dict, description="Mapping of category names to lists of tool names"
    )
        ...
        if self.use_rag:
            ...
            if isinstance(self.retriever_config, VectorStoreConfig):
                self.retriever_config = VectorStoreRetrieverConfig(
                    name=f"retriever_for_{self.name}", vector_store_config=self.retriever_config
                )
        return self
