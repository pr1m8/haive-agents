...
class ReactAgentConfig(AgentConfig):
    ...
    # Tools configuration
    tools: List[Union[BaseTool, StructuredTool, Tool]] = Field(
    ...
    agent_node_name: str = Field(
        default="agent_node",
        description="Name for the agent node in the graph."
    )
    
    tool_node_name: str = Field(
        default="tool_node",
        description="Name for the tool node in the graph."
    )
    ...
    routing_function: Optional[Callable] = Field(
        default=None,
        description="Custom function to determine routing between nodes."
    )
    ...
    use_memory: bool = Field(
        default=True,
        description="Whether to use memory for state persistence."
    )
    ...
    def align_output_format(cls, v, info):
        ...
        elif info.field_name == "response_format" and v is None:
