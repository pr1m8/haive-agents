...
class ReactAgent(SimpleAgent):
    ...
    def setup_workflow(self) -> None:
        ...
        router_node_config = NodeConfig(
            ...
            engine=self._create_router_function()
        )
        ...
        if self.config.tool_routing and self.config.version == "v2":
            for tool_name, destination in self.config.tool_routing.items():
                if destination not in route_destinations and destination != self.config.tool_node_name:
    ...
    def _route_based_on_messages(self, state: ReactAgentSchema) -> Union[str, Send, List[Send]]:
        """
    ...
    def _create_structured_output_node(self) -> Callable:
        ...
        def generate_structured_response(state: ReactAgentSchema) -> Dict[str, Any]:
            ...
            try:
                ...
                model_name = self.config.response_format.__name__ if hasattr(self.config.response_format, "__name__") else "Structured"
    ...
    def run(self, input_data: Any) -> Dict[str, Any]:
        ...
        if self.config.save_history:
            try:
                ...
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                state_filename = os.path.join(output_dir, f"{self.config.name}_{timestamp}.json")
