...
class ReactAgentConfig(AgentConfig):
    ...
    tools: list[BaseTool] = Field(default_factory=list, description='List of tools that the agent can use')
    state_schema: type[StateSchema] | None = Field(default=None, description='Schema for agent state (auto-derived if not provided)')
    ...
    system_prompt: str = Field(default='You are a helpful assistant with access to tools. When you need information or need to perform an action, use the appropriate tool. First think about what you need to accomplish, then select the right tool for the task.', description='System prompt for the agent')
    reasoning_retry: RetryPolicy | None = Field(default=None, description='Retry policy for reasoning node')
    tool_retry: RetryPolicy | None = Field(default=None, description='Retry policy for tool execution node')
    ...
    include_tool_names_in_prompt: bool = Field(default=True, description='Whether to explicitly include tool names in the system prompt')
    ...
    def setup_defaults(self) -> Any:
        ...
        if self.reasoning_retry is None:
            self.reasoning_retry = RetryPolicy(initial_interval=1.0, backoff_factor=2.0, max_interval=16.0, max_attempts=3, jitter=True)
        if self.tool_retry is None:
            self.tool_retry = RetryPolicy(initial_interval=0.5, backoff_factor=1.5, max_interval=8.0, max_attempts=2, jitter=True)
