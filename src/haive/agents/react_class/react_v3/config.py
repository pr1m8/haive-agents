...
class ReactAgentConfig(AgentConfig):
    ...
    engine: AugLLMConfig = Field(
        description="The LLM engine to use for reasoning"
    )
    ...
    reasoning_retry: Optional[RetryPolicy] = Field(
        default=None,
        description="Retry policy for reasoning node"
    )
    
    tool_retry: Optional[RetryPolicy] = Field(
        default=None,
        description="Retry policy for tool execution node"
    )
    
    # Maximum iterations
    max_iterations: int = Field(
        default=10,
        description="Maximum number of reasoning-tool cycles"
    )
