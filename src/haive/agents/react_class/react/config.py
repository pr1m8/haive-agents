...
class ReactAgentConfig(SimpleAgentConfig):
    ...
    tools: list[BaseTool | dict[str, Any]] = Field(default_factory=list, description='Tools available to the agent')
    max_iterations: int = Field(default=10, description='Maximum number of iterations for the agent')
    structured_output_schema: type[BaseModel] | None = Field(default=None, description='Schema for structured output')
    system_prompt: str | None = Field(default=None, description='System prompt for the agent')
    tool_choice: Literal['auto', 'any', 'none'] | dict[str, Any] | None = Field(default='auto', description='Tool choice configuration for the LLM')
