...
class SearchContext(BaseModel):
    ...
    current_date: datetime = Field(default_factory=datetime.now, description='Current date and time for temporal context')
    user_location: str | None = Field(default=None, description="User's location for geo-specific searches")
    search_history: list[str] = Field(default_factory=list, description='Recent search queries for context')
    domain_preferences: set[str] = Field(default_factory=set, description='Preferred domains or sources')
    ...
    def temporal_context(self) -> dict[str, str]:
        ...
        return {'date': now.strftime('%Y-%m-%d'), 'time': now.strftime('%H:%M:%S'), 'day_of_week': now.strftime('%A'), 'month_year': now.strftime('%B %Y'), 'relative_time': self._get_relative_time()}
...
class QueryIntent(BaseModel):
    ...
    intent_type: Literal['factual', 'comparison', 'how-to', 'definition', 'current_events', 'analysis', 'recommendation', 'navigation'] = Field(description='Primary intent of the query')
    temporal_scope: Literal['historical', 'current', 'future', 'timeless'] = Field(default='timeless', description='Temporal relevance of the query')
    complexity_level: Literal['simple', 'moderate', 'complex'] = Field(default='moderate', description='Estimated complexity of the query')
    required_sources: int = Field(default=3, ge=1, le=10, description='Estimated number of sources needed')
    key_entities: list[str] = Field(default_factory=list, description='Key entities identified in the query')
    related_concepts: list[str] = Field(default_factory=list, description='Related concepts that might enhance search')
...
class QueryReasoning(BaseModel):
    ...
    understanding: str = Field(description='Natural language understanding of what the user is asking')
    search_strategy: str = Field(description='Strategy for searching this query effectively')
    potential_challenges: list[str] = Field(default_factory=list, description='Potential challenges in finding accurate information')
...
class SearchQueryConfig(BaseModel):
    ...
    query_type: Literal['primary', 'supporting', 'verification', 'expansion'] = Field(default='primary', description='Type/purpose of this query')
    target_source_types: list[Literal['web', 'academic', 'news', 'wiki', 'social', 'video', 'image']] = Field(default_factory=lambda: ['web'], description='Preferred source types for this query')
    expected_result_type: Literal['facts', 'list', 'explanation', 'comparison', 'tutorial', 'mixed'] = Field(default='mixed', description='Expected type of results')
...
class QueryBatch(BaseModel):
    ...
    queries: list[SearchQueryConfig] = Field(min_length=1, max_length=10, description='List of queries to execute')
    execution_strategy: Literal['parallel', 'sequential', 'priority'] = Field(default='parallel', description='How to execute these queries')
...
class SearchResult(BaseModel):
    ...
    source_type: Literal['web', 'academic', 'news', 'wiki', 'social', 'video', 'image'] = Field(default='web')
...
class ContentAnalysis(BaseModel):
    ...
    key_findings: list[str] = Field(min_length=1, description='Key findings from the search results')
    common_themes: list[str] = Field(default_factory=list, description='Common themes across results')
    contradictions: list[dict[str, str]] = Field(default_factory=list, description='Contradictions found between sources')
    confidence_level: Literal['high', 'medium', 'low'] = Field(default='medium', description='Confidence in the findings')
    gaps_identified: list[str] = Field(default_factory=list, description='Information gaps that remain')
...
class SearchSynthesis(BaseModel):
    ...
    answer_completeness: float = Field(ge=0.0, le=1.0, description='How completely the search answered the query')
    follow_up_queries: list[str] = Field(default_factory=list, max_length=5, description='Suggested follow-up queries')
    citations: list[dict[str, str]] = Field(default_factory=list, description='Citations for key claims')
    ...
    def requires_follow_up(self) -> bool:
        ...
        return self.answer_completeness < 0.7 or len(self.analysis.gaps_identified) > 2 or self.analysis.confidence_level == 'low'
        ...
        if not self.citations and self.total_sources_used > 0:
            for result in self.search_results[:3]:
                if result.success and result.top_results:
                    ...
                    self.citations.append({'claim': f'Information about {result.query.query_text}', 'source': top.title, 'url': top.url})
        return self
...
class PerplexitySearchState(BaseModel):
    ...
    reasoning: QueryReasoning | None = Field(default=None, description='Query reasoning and understanding')
    query_batch: QueryBatch | None = Field(default=None, description='Generated search queries')
    search_results: list[SearchQueryResult] = Field(default_factory=list, description='Raw search results')
    ...
    def is_complete(self) -> bool:
        ...
        return self.synthesis is not None and (not self.synthesis.requires_follow_up) or self.iteration_count >= self.max_iterations

    @computed_field
    @property
    def next_action(self) -> Literal['reason', 'generate_queries', 'search', 'synthesize', 'complete']:
