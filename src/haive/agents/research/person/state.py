...
class PersonResearchInputState(BaseModel):
    ...
    user_notes: Optional[Dict[str, Any]] = Field(
        default=None,
...
class PersonResearchState(BaseModel):
    ...
    is_satisfactory: Optional[bool] = Field(
        default=None,
        description="True if all required fields are well populated, False otherwise"
    )
    reflection_steps_taken: int = Field(
        default=0,
        description="Number of times the reflection node has been executed"
    )
...
class PersonResearchAgentConfig(BaseModel):
    ...
    max_search_queries: int = Field(
        default=3,
        description="Maximum number of search queries per person"
    )
    ...
    max_reflection_steps: int = Field(
        default=0,
        description="Maximum number of reflection steps"
    )