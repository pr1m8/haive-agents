...
from haive.core.engine.agent.config import ReactAgentConfig
...
class ResearchAgent(Agent[ResearchAgentConfig]):
    ...
    def _discover_document_loaders(self) -> Dict[str, Any]:
        ...
        loader_classes = {}
        for _, module_name, _ in pkgutil.walk_packages(base_loader_pkg.__path__, base_loader_pkg.__name__ + "."):
            try:
    ...
    def generate_search_queries(self, state: ResearchState) -> Command:
        ...
        if state.current_section_index is None or state.current_section_index >= len(state.report_sections):
    ...
    def recommend_document_loaders(self, state: ResearchState) -> Command:
        ...
        state.data_sources = [{
            "type": source,
            "recommended_loaders": [loader for loader in loader_mapping.get(source, []) 
        ...
        for i, query in enumerate(state.search_queries):
            ...
            try:
                ...
                if data_source == "web":
                    ...
                    loader = self._create_document_loader("ArxivLoader", query=query_text, load_max_docs=3)
                    documents = loader.load()
    ...
    def evaluate_sources(self, state: ResearchState) -> Command:
        ...
        if state.current_section_index is not None and state.current_section_index < len(state.report_sections):
            current_section = state.report_sections[state.current_section_index]
    ...
    def check_section_completion(self, state: ResearchState) -> str:
        ...
        next_index = None
        for i, section in enumerate(state.report_sections):
            if i > state.current_section_index and section.requires_research and section.status != "completed":
                next_index = i
                break
    ...
    def consolidate_findings(self, state: ResearchState) -> Command:
        ...
        for section in state.report_sections:
            ...
            try:
                ...
                logger.error(f"Error extracting findings from section {section.name}: {e}")
    ...
    def assess_confidence(self, state: ResearchState) -> Command:
        ...
        high_reliability_sources = sum(1 for source in state.sources if source.get("reliability") == "HIGH")
        recent_sources = sum(1 for source in state.sources 
    ...
    def compile_final_report(self, state: ResearchState) -> Command:
        ...
        output_message = AIMessage(content=f"""
    ...
    def visualize_state(self, state: Dict[str, Any]) -> None:
        ...
        print("\n" + "=" * 50)
        print(f"RESEARCH STATE VISUALIZATION")
        print("=" * 50)
