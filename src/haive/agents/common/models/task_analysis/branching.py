...
class TaskBranch(BaseModel):
    ...
    branch_id: str = Field(..., description='Unique identifier for this branch', min_length=1, max_length=100, examples=['find_winner', 'calculate_sum', 'research_mechanism', 'validate_results'])
    name: str = Field(..., description='Human-readable name for the branch', min_length=1, max_length=200, examples=['Find Recent Wimbledon Winner', 'Calculate Mathematical Sum', 'Research Cancer Mechanisms', 'Validate Experimental Results'])
    description: str = Field(..., description='Detailed description of what this branch accomplishes', min_length=10, max_length=1000, examples=['Look up the most recent Wimbledon championship winner using sports databases', 'Perform mathematical calculations on retrieved factual data', 'Conduct comprehensive research into cellular mechanisms of cancer development'])
    branch_type: BranchType = Field(..., description='Type of execution pattern for this branch', examples=['sequential', 'parallel', 'conditional', 'iterative'])
    estimated_effort: int = Field(..., description='Relative effort required on 1-10 scale (1=trivial, 10=maximum)', ge=1, le=10, examples=[1, 3, 7, 10])
    estimated_duration: timedelta = Field(..., description='Expected time to complete this branch', examples=[timedelta(minutes=5), timedelta(hours=2), timedelta(days=30), timedelta(weeks=52)])
    prerequisites: list[str] = Field(default_factory=list, description='IDs of other branches that must complete first', examples=[[], ['find_winner'], ['literature_review', 'lab_setup'], ['data_collection', 'analysis_plan']])
    enables: list[str] = Field(default_factory=list, description='IDs of branches that this branch enables', examples=[['find_birthday'], ['calculate_result'], ['clinical_trials', 'drug_development'], ['publication', 'patent_filing']])
    resources_needed: list[str] = Field(default_factory=list, description='Specific resources required for this branch', examples=[['web_search', 'sports_database'], ['calculator', 'mathematical_knowledge'], ['research_lab', 'expert_oncologists', 'funding'], ['supercomputer', 'quantum_hardware', 'specialist_team']])
    parallel_compatible: bool = Field(default=True, description='Whether this branch can run in parallel with others', examples=[True, False])
    risk_level: int = Field(default=1, description='Risk level for this branch (1=low, 5=extreme)', ge=1, le=5, examples=[1, 2, 3, 4, 5])
    success_probability: float = Field(default=0.9, description='Estimated probability of successful completion', ge=0.0, le=1.0, examples=[0.95, 0.8, 0.3, 0.1])
...
class TaskDecomposition(BaseModel):
    ...
    """
    task_description: str = Field(..., description='Original task being decomposed', min_length=10, max_length=1000, examples=['Find the birthday of the most recent Wimbledon winner', "Calculate Tom Clancy's birthday + sun's age + feet in mile, then square result", 'Develop a cure for cancer through novel therapeutic approaches'])
    branches: list[TaskBranch] = Field(..., description='List of individual execution branches', min_length=1, max_length=50)
    execution_pattern: str = Field(..., description='Overall execution pattern', examples=['linear_sequential', 'parallel_convergent', 'complex_dependent', 'iterative_research', 'branching_exploration', 'pipeline_flow'])
    critical_path: list[str] = Field(..., description='Sequence of branch IDs on the critical path', examples=[['find_winner', 'find_birthday'], ['research_mechanism', 'develop_therapy', 'clinical_trials'], ['gather_facts', 'calculate', 'square_result']])
    parallelization_opportunities: list[list[str]] = Field(default_factory=list, description='Groups of branch IDs that can run in parallel', examples=[[['find_clancy_birthday', 'find_sun_age', 'find_mile_feet']], [['mechanism_research', 'drug_screening'], ['toxicity_testing', 'efficacy_testing']], [['data_source_1', 'data_source_2', 'data_source_3']]])
    bottlenecks: list[str] = Field(default_factory=list, description='Branch IDs that are likely to be bottlenecks', examples=[['expert_review'], ['regulatory_approval', 'funding_acquisition'], ['specialized_equipment_access']])
    total_estimated_effort: int = Field(..., description='Sum of all branch effort estimates', ge=1, examples=[5, 25, 150, 500])
    estimated_duration_sequential: timedelta = Field(..., description='Total duration if executed sequentially', examples=[timedelta(minutes=15), timedelta(hours=8), timedelta(weeks=104), timedelta(weeks=520)])
    estimated_duration_optimal: timedelta = Field(..., description='Duration with optimal parallelization', examples=[timedelta(minutes=10), timedelta(hours=4), timedelta(weeks=52), timedelta(weeks=260)])
    ...
    def validate_decomposition_consistency(self) -> 'TaskDecomposition':
        ...
        if abs(calculated_effort - self.total_estimated_effort) > 1:
            raise ValueError(f"Total effort {self.total_estimated_effort} doesn't match sum of branches {calculated_effort}")
    ...
    def get_complexity_metrics(self) -> dict[str, Any]:
        ...
        """
        return {'total_branches': len(self.branches), 'total_effort': self.total_estimated_effort, 'average_effort_per_branch': self.total_estimated_effort / len(self.branches), 'critical_path_length': len(self.critical_path), 'parallelizable_groups': len(self.parallelization_opportunities), 'bottleneck_count': len(self.bottlenecks), 'max_branch_effort': max((branch.estimated_effort for branch in self.branches)), 'dependency_density': sum((len(branch.prerequisites) for branch in self.branches)) / len(self.branches), 'parallelization_speedup': self.calculate_parallelization_speedup(), 'high_risk_branches': sum((1 for branch in self.branches if branch.is_high_risk())), 'low_success_probability': sum((1 for branch in self.branches if not branch.is_likely_to_succeed()))}
    ...
    def get_execution_recommendations(self) -> list[str]:
        ...
        if metrics['parallelization_speedup'] > 2.0:
            recommendations.append('High parallelization potential - strongly recommend parallel execution')
        ...
        if metrics['bottleneck_count'] > 0:
            recommendations.append(f'Address {metrics['bottleneck_count']} identified bottlenecks early')
        if metrics['high_risk_branches'] > 0:
            recommendations.append(f'Plan mitigation for {metrics['high_risk_branches']} high-risk branches')
        if metrics['low_success_probability'] > 0:
            recommendations.append(f'Consider alternatives for {metrics['low_success_probability']} low-probability branches')
    ...
    @classmethod
    def create_simple_sequential(cls, task_description: str, branch_descriptions: list[str], effort_estimates: list[int] | None=None, duration_estimates: list[timedelta] | None=None) -> 'TaskDecomposition':
        ...
        for i, description in enumerate(branch_descriptions):
            ...
            branch = TaskBranch(branch_id=branch_id, name=f'Step {i + 1}', description=description, branch_type=BranchType.SEQUENTIAL, estimated_effort=effort_estimates[i], estimated_duration=duration_estimates[i], prerequisites=prerequisites, enables=enables, parallel_compatible=False)
        ...
        return cls(task_description=task_description, branches=branches, execution_pattern='linear_sequential', critical_path=critical_path, parallelization_opportunities=[], bottlenecks=[], total_estimated_effort=total_effort, estimated_duration_sequential=sequential_duration, estimated_duration_optimal=sequential_duration)