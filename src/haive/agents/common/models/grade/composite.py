...
class CompositeGrade(Grade):
    ...
    """
    grade_type: GradeType = Field(default=GradeType.COMPOSITE, description='Type of grade model (always composite)')
    grades: list[Grade] = Field(..., description='List of individual grades to combine', min_length=2, max_length=10)
    weights: list[float] | None = Field(default=None, description='Optional weights for each grade (auto-normalized if provided)', examples=[[0.3, 0.4, 0.3], [1.0, 2.0, 1.0], [0.25, 0.25, 0.5]])
    combination_method: str = Field(default='weighted_average', description='Method for combining grades', examples=['weighted_average', 'simple_average', 'median', 'conservative', 'optimistic'])
    primary_grade_index: int | None = Field(default=None, description='Index of the primary/most important grade (0-based)', examples=[0, 1, 2])
    consensus_threshold: float = Field(default=0.8, description='Threshold for determining grade consensus (0.0 to 1.0)', ge=0.0, le=1.0)
    ...
    def validate_combination_method(cls, v: str) -> str:
        ...
        """
        valid_methods = {'weighted_average', 'simple_average', 'median', 'conservative', 'optimistic', 'consensus', 'primary_with_validation'}
    ...
    def validate_weights_and_indices(self) -> 'CompositeGrade':
        ...
        if self.weights is not None:
            if len(self.weights) != len(self.grades):
                raise ValueError(f'Number of weights ({len(self.weights)}) must match number of grades ({len(self.grades)})')
        ...
        if self.primary_grade_index is not None:
            if not 0 <= self.primary_grade_index < len(self.grades):
                raise ValueError(f'primary_grade_index must be between 0 and {len(self.grades) - 1}')
        return self
    ...
    def get_normalized_score(self) -> float:
        ...
        if self.combination_method == 'primary_with_validation':
            if self.primary_grade_index is not None:
                ...
                avg_others = sum((scores[i] for i in range(len(scores)) if i != self.primary_grade_index)) / (len(scores) - 1)
    ...
    def get_grade_statistics(self) -> dict[str, float]:
        ...
        return {'mean': mean_score, 'median': median, 'min': min(scores), 'max': max(scores), 'std_dev': std_dev, 'variance': variance, 'range': max(scores) - min(scores), 'cv': std_dev / mean_score if mean_score > 0 else 0, 'consensus': self.has_consensus()}
    ...
    def get_grade_breakdown(self) -> list[dict[str, Any]]:
        ...
        for i, grade in enumerate(self.grades):
            breakdown.append({'index': i, 'grade_type': grade.grade_type.value, 'normalized_score': grade.get_normalized_score(), 'weight': weights[i], 'weighted_contribution': grade.get_normalized_score() * weights[i], 'is_primary': i == self.primary_grade_index, 'is_passing': grade.is_passing(), 'justification_preview': grade.justification[:50] + '...' if len(grade.justification) > 50 else grade.justification})
        return breakdown
    ...
    def get_consensus_analysis(self) -> dict[str, Any]:
        ...
        consensus_level = 'high' if stats.get('cv', 1.0) < 0.1 else 'medium' if stats.get('cv', 1.0) < 0.3 else 'low'
        return {'has_consensus': self.has_consensus(), 'consensus_level': consensus_level, 'coefficient_of_variation': stats.get('cv', 0), 'score_range': stats.get('range', 0), 'outlier_count': len(outliers), 'outlier_indices': outliers, 'agreement_percentage': (1.0 - stats.get('cv', 1.0)) * 100}
    ...
    @classmethod
    def create_from_grades(cls, grades: list[Grade], justification: str, weights: list[float] | None=None, method: str='weighted_average', **kwargs) -> 'CompositeGrade':
        ...
        """
        return cls(grades=grades, weights=weights, combination_method=method, justification=justification, **kwargs)

    @classmethod
    def create_consensus_grade(cls, grades: list[Grade], justification: str, consensus_threshold: float=0.8, **kwargs) -> 'CompositeGrade':
        ...
        """
        return cls(grades=grades, combination_method='consensus', consensus_threshold=consensus_threshold, justification=justification, **kwargs)