...
class MultiAgentV2(Agent):
    ...
    """
    agents: list[Agent] | dict[str, Agent] = Field(..., description='Agents to coordinate (list or dict)')
    execution_mode: ExecutionMode = Field(default=ExecutionMode.SEQUENCE, description='How agents are executed')
    state_schema: type[StateSchema] = Field(default=MultiAgentState, description='State schema (defaults to MultiAgentState)')
    use_prebuilt_base: bool = Field(default=True, description='Use MultiAgentState as base for composition')
    routing_function: Callable | None = Field(default=None, description='Function for conditional routing')
    route_map: dict[str, str] | None = Field(default=None, description='Map of routing outputs to agent names')
    ...
    @classmethod
    def from_agents(cls, agents: list[Agent] | dict[str, Agent], name: str | None=None, execution_mode: ExecutionMode=ExecutionMode.SEQUENCE, **kwargs) -> 'MultiAgentV2':
        ...
        """
        return cls(name=name or 'multi_agent', agents=agents, execution_mode=execution_mode, **kwargs)

    @classmethod
    def from_config(cls, config: dict[str, Any], agents: list[Agent] | dict[str, Agent] | None=None) -> 'MultiAgentV2':
    ...
    @classmethod
    def rebuild_with_agents(cls, original: 'MultiAgentV2', new_agents: list[Agent] | dict[str, Agent], **kwargs) -> 'MultiAgentV2':
        ...
        """
        config = {'name': original.name, 'execution_mode': original.execution_mode, 'state_schema': original.state_schema, 'routing_function': original.routing_function, 'route_map': original.route_map}
    ...
    def _build_sequence_graph(self, graph: BaseGraph) -> None:
        ...
        for i, agent_name in enumerate(agent_names):
            node = AgentNodeV3Config(name=f'agent_{agent_name}', agent_name=agent_name, project_state=True, extract_from_container=True, update_container_state=True)
    ...
    def _build_conditional_graph(self, graph: BaseGraph) -> None:
        ...
        for route_key, agent_name in self.route_map.items():
            node = AgentNodeV3Config(name=f'agent_{agent_name}', agent_name=agent_name, project_state=True, extract_from_container=True, update_container_state=True)
            graph.add_node(f'agent_{agent_name}', node)
            graph.add_conditional_edges('router', self.routing_function, {route_key: f'agent_{agent_name}'})
