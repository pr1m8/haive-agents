...
class MultiAgent(Agent):
    ...
    """
    agents: list[Agent] | dict[str, Agent] = Field(..., description='Agents to coordinate - can be list or dict like engines')
    mode: Literal['sequential', 'conditional', 'parallel'] = Field(default='sequential', description='Execution mode for agents')
    state_strategy: Literal['minimal', 'container', 'custom'] = Field(default='minimal', description='State management strategy')
    shared_fields: list[str] = Field(default_factory=list, description='Fields shared between agents (empty for private state passing)')
    state_transfer_map: dict[str, dict[str, str]] = Field(default_factory=dict, description='Maps agent outputs to next agent inputs')
    routing_function: Any | None = Field(default=None, description='Function for conditional routing')
    ...
    def build_graph(self) -> BaseGraph:
        ...
        for agent_name, agent in self._agent_registry.items():
            node_config = AgentNodeV3Config(name=f'agent_{agent_name}', agent_name=agent_name, agent=agent, project_state=True, extract_from_container=self.state_strategy == 'container', shared_fields=self.shared_fields, update_container_state=self.state_strategy == 'container')
    ...
    def _build_conditional_edges(self, graph: BaseGraph):
        ...
        for i, agent_name in enumerate(self._agent_order[:-1]):
            node_name = f'agent_{agent_name}'
            route_map = {next_agent: f'agent_{next_agent}' for next_agent in self._agent_order[i + 1:]}
            route_map[END] = END
...
class ConditionalAgent(MultiAgent):
    ...
    def __init__(self, agents: dict[str, Agent], routing_function: Any, **kwargs):
        ...
        super().__init__(agents=agents, mode='conditional', routing_function=routing_function, **kwargs)