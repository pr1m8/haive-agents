...
class AgentBranch:
    ...
    def __init__(self, from_agent: str | Agent, condition: Callable[[Any], str], destinations: dict[str, str | Agent], default: str | Agent | None=None):
...
class WorkflowStep:
    ...
    def __init__(self, name: str, function: Callable, inputs: list[str | Agent] | None=None, outputs: list[str | Agent] | None=None):
...
class ConfigurableMultiAgent(Agent):
    ...
    branches: list[AgentBranch] = Field(default_factory=list, description='Branches/routing between agents')
    workflow_steps: list[WorkflowStep] = Field(default_factory=list, description='Workflow steps between agents')
    state_schema_override: type[StateSchema] | None = Field(default=None, description='Optional state schema override')
    schema_composition_method: str = Field(default='smart', description='Schema composition strategy')
    include_meta: bool = Field(default=True, description='Include meta state for coordination')
    start_agent: str | Agent | None = Field(default=None, description='Agent to start execution with')
    end_condition: Callable[[Any], bool] | None = Field(default=None, description='Function to determine when to end')
    ...
    def setup_configurable_multi_agent(self) -> 'ConfigurableMultiAgent':
        ...
        if self.state_schema_override:
            ...
            self.state_schema = AgentSchemaComposer.from_agents(agents=self.agents, name=f'{self.__class__.__name__}State', include_meta=self.include_meta, separation=self.schema_composition_method, build_mode=BuildMode.CUSTOM)
    ...
    def add_branch(self, from_agent: str | Agent, condition: Callable[[Any], str], destinations: dict[str, str | Agent], default: str | Agent | None=None) -> None:
        """Add a branch/routing between agents."""
        branch = AgentBranch(from_agent, condition, destinations, default)
        self.branches.append(branch)

    def add_workflow_step(self, name: str, function: Callable, inputs: list[str | Agent] | None=None, outputs: list[str | Agent] | None=None) -> None:
    ...
    def build_graph(self) -> BaseGraph:
        ...
        for branch in self.branches:
            ...
            graph.add_conditional_edges(source_node, branch.condition, normalized_destinations, default=default_dest)
        if self.start_agent:
...
def create_sequential_multi_agent(agents: list[Agent], name: str='SequentialMultiAgent', state_schema: type[StateSchema] | None=None, **kwargs) -> ConfigurableMultiAgent:
    """Create a sequential multi-agent system."""
    return ConfigurableMultiAgent(name=name, agents=agents, state_schema_override=state_schema, **kwargs)

def create_branching_multi_agent(agents: list[Agent], branches: list[AgentBranch], name: str='BranchingMultiAgent', state_schema: type[StateSchema] | None=None, **kwargs) -> ConfigurableMultiAgent:
    """Create a multi-agent system with conditional branches."""
    return ConfigurableMultiAgent(name=name, agents=agents, branches=branches, state_schema_override=state_schema, **kwargs)

def create_workflow_multi_agent(agents: list[Agent], workflow_steps: list[WorkflowStep], name: str='WorkflowMultiAgent', state_schema: type[StateSchema] | None=None, **kwargs) -> ConfigurableMultiAgent:
    """Create a multi-agent system with workflow steps."""
    return ConfigurableMultiAgent(name=name, agents=agents, workflow_steps=workflow_steps, state_schema_override=state_schema, **kwargs)