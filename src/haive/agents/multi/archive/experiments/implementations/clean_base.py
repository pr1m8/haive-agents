...
class CleanMultiAgent(Agent):
    ...
    """
    agents: list[Agent] | dict[str, Agent] = Field(default_factory=list, description='Agents to coordinate - similar to engines field')
    execution_mode: str = Field(default='sequential', description='How to execute agents: sequential, parallel, conditional')
    ...
    def build_graph(self) -> BaseGraph:
        ...
        for agent_name, agent in self.agents.items():
            node_config = AgentNodeV3Config(name=f'agent_{agent_name}', agent_name=agent_name, agent=agent, project_state=True, extract_from_container=True, update_container_state=True)
