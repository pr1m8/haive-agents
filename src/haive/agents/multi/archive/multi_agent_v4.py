...
class MultiAgentV4(Agent):
    ...
    """
    agents: list[Agent] = Field(default_factory=list, description='List of agents to coordinate (converted to dict internally)')
    execution_mode: Literal['sequential', 'parallel'] = Field(default='sequential', description='How to execute the agents')
    build_mode: Literal['auto', 'manual', 'lazy'] = Field(default='auto', description='When to build the execution graph')
    agent_dict: dict[str, Agent] = Field(default_factory=dict, description='Internal agent dictionary (converted from list)')
    execution_graph: CompiledGraph | None = Field(default=None, description='Compiled LangGraph for execution')
    state_schema: type[MultiAgentState] = Field(default=MultiAgentState, description='State schema to use')
    ...
    def setup_multi_agent(self):
        ...
        if self.agents:
            ...
            logger.info(f'Converted {len(self.agents)} agents to dict: {list(self.agent_dict.keys())}')
    ...
    def _build_execution_graph(self) -> CompiledGraph:
        ...
        logger.info(f'Building {self.execution_mode} execution graph for {len(self.agent_dict)} agents')
