...
class DynamicSupervisor(SupervisorAgent):
    ...
    def assign_task(self, task_id: str, worker_name: Optional[str] = None) -> Optional[str]:
    ...
    def get_worker_metrics(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            name: {
                **metrics,
                "average_time": metrics["total_time"] / metrics["tasks_completed"] 
...
if __name__ == "__main__":
    ...
    for name, metrics in supervisor.get_worker_metrics().items():
        print(f"  {name}: success_rate={metrics['success_rate']:.2f}, avg_time={metrics['average_time']:.2f}s")