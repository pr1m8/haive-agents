...
def create_relationship_extraction_config(
    ...
    relationship_extraction_prompt = ChatPromptTemplate.from_messages([
        ("system", """You are an expert relationship extractor. 
...
def create_graph_merger_config(
    ...
    graph_merge_prompt = ChatPromptTemplate.from_messages([
        ("system", """You are an expert knowledge graph reconciliation specialist.
        ...
        ("human", """Merge these knowledge graphs with precision:
        {graph_contexts}
...
def main():
    ...
    for name, config in configs.items():
        ...
        print(f"  Temperature: {config.llm_config.parameters.get('temperature', 'N/A')}")
        print(f"  Structured Output: {config.structured_output_model.__name__ if config.structured_output_model else 'None'}")
        print()
