...
class EntityRelationship(BaseModel):
    ...
    confidence_score: float = Field(default=0.5, ge=0, le=1, description='Confidence score for the relationship')
    supporting_evidence: str | None = Field(default=None, description='Textual evidence supporting the relationship')
    ...
    @classmethod
    def from_graph_relationship(cls, relationship: Relationship, confidence_score: float=0.5, supporting_evidence: str | None=None):
        ...
        """
        return cls(source=relationship.source, target=relationship.target, type=relationship.type, properties=relationship.properties or {}, confidence_score=confidence_score, supporting_evidence=supporting_evidence)

class KnowledgeGraph(BaseModel):
    """Represents a comprehensive knowledge graph."""
    nodes: list[EntityNode] = Field(default_factory=list, description='Nodes in the knowledge graph')
    relationships: list[EntityRelationship] = Field(default_factory=list, description='Relationships in the knowledge graph')
    ...
    def add_relationship(self, relationship: EntityRelationship):
        ...
        """
        if not any((existing.source == relationship.source and existing.target == relationship.target and (existing.type == relationship.type) for existing in self.relationships)):
...
def main() -> None:
    person_node = EntityNode(id='marie_curie', type='Scientist', properties={'name': 'Marie Curie', 'nationality': 'Polish-French', 'field': 'Physics and Chemistry'})
    country_node = EntityNode(id='poland', type='Country', properties={'name': 'Poland', 'continent': 'Europe'})
    relationship = EntityRelationship(source='marie_curie', target='poland', type='BORN_IN', confidence_score=0.9, supporting_evidence='Marie Curie was born in Warsaw, Poland in 1867')
    kg = KnowledgeGraph()
