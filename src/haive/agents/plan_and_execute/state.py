...
class PlanAndExecuteState(BaseModel):
    ...
    plan: Plan | None = Field(default=None, description="The current plan, including tasks and subtasks.")
    past_steps: list[Step] = Field(default_factory=list, description="A list of completed steps or actions.")
    response: str | None = Field(default=None, description="The current response or output from the agent.")
