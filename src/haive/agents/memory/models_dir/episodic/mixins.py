...
class PerformanceMetrics(BaseModel):
    ...
    completion_time: float = Field(default=0.0, ge=0.0, description='Average completion time in seconds')
    user_satisfaction: float | None = Field(None, ge=1.0, le=5.0, description='User satisfaction score (1-5)')
...
class TaskExecution(BaseModel):
    ...
    input_parameters: dict[str, Any] = Field(default_factory=dict, description='Task input parameters')
    execution_steps: list[str] = Field(default_factory=list, description='Step-by-step execution log')
    tools_used: list[str] = Field(default_factory=list, description='Tools utilized during execution')
