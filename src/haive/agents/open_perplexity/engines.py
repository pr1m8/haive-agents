...
from haive.agents.open_perplexity.models import ResearchFinding, ResearchSource, ResearchSummary
from haive.agents.open_perplexity.prompts import (
...
def create_research_engines(model_name: str = "gpt-4o") -> dict[str, AugLLMConfig]:
    """Create all engines needed for research agent"""
    engines = {}

    # Topic extraction engine
    topic_extraction_template = ChatPromptTemplate.from_messages([
        SystemMessage(content="You are an information extraction assistant that identifies research topics and questions from user input."),
    ...
    engines["query_generation"] = AugLLMConfig(
        ...
        llm_config=create_base_llm_config(model_name, 0.3),  # Higher temp for creative queries
    ...
    engines["source_evaluation"] = AugLLMConfig(
        ...
        llm_config=create_base_llm_config(model_name, 0.1),  # Lower temp for objective evaluation
