...
```bash
python -m src.haive.agents.open_perplexity.cli research path/to/question.txt --output report.md --save-state history.json
```

Visualize a saved state:

```bash
python -m src.haive.agents.open_perplexity.cli visualize history.json --markdown report.md
```

### Python API

```python
from src.haive.agents.open_perplexity import ResearchAgent, ResearchAgentConfig
