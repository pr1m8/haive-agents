...
from haive.agents.open_perplexity.engines import create_research_engines
from haive.agents.open_perplexity.prompts import MAIN_SYSTEM_PROMPT
from haive.agents.open_perplexity.react_agent_config import (
...
class ResearchAgentConfig(AgentConfig):
    ...
    research_depth: int = Field(
        default=3,
        description="Depth of research (1-5, higher means more thorough)"
    )

    max_sources_per_query: int = Field(
        default=5,
        description="Maximum number of sources to use per query"
    )

    concurrent_searches: int = Field(
        default=3,
        description="Number of concurrent searches to perform"
    )
