...
class AgentEntry(BaseModel):
    ...
    def from_agent(cls, name: str, description: str, agent: Agent) -> 'AgentEntry':
        ...
        return cls(name=name, description=description, agent_instance=pickle.dumps(agent), agent_class=agent.__class__.__name__)
...
class SupervisorReactState(StateSchema):
    ...
    registered_agents: dict[str, AgentEntry] = Field(default_factory=dict, description='Registered agents stored as serialized entries')
    handoff_tools: dict[str, BaseTool] = Field(default_factory=dict, description='Handoff tools mapped to agent names')
    ...
    def sync_tools_with_agents(self) -> 'SupervisorReactState':
        ...
        for agent_name, agent_entry in self.registered_agents.items():
            if agent_name not in self.handoff_tools:
                tool = create_handoff_tool(agent_name=agent_name, description=agent_entry.description)
...
class StaticSupervisor(ReactAgent[SupervisorReactState]):
    ...
    def _execute_agent_handoff(self, state: SupervisorReactState, agent_name: str, tool_call: dict) -> str:
    ...
    def _execute_regular_tool(self, state: SupervisorReactState, tool_name: str, tool_call: dict) -> str:
