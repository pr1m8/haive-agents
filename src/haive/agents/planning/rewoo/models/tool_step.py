...
class ToolStep(AbstractStep):
    ...
    available_tools: list[BaseTool] = Field(..., description='List of available tools for validation')
    ...
    def tool_schema(self) -> dict[str, Any] | None:
        ...
        if self.selected_tool:
            return self.selected_tool.args_schema.model_json_schema() if self.selected_tool.args_schema else None
        return None
    ...
    def validate_tool_exists_and_args(self) -> 'ToolStep':
        ...
        if self.tool_name not in self.tool_names:
            raise ValueError(f"Tool '{self.tool_name}' not found. Available tools: {', '.join(self.tool_names)}")
        if not self._are_args_valid():
            missing_args = set(self.required_args) - set(self.tool_args.keys())
            if missing_args:
                raise ValueError(f"Missing required arguments for tool '{self.tool_name}': {', '.join(missing_args)}")
            valid_args = set(self.required_args + self.optional_args)
            invalid_args = set(self.tool_args.keys()) - valid_args
            if invalid_args:
                raise ValueError(f"Invalid arguments for tool '{self.tool_name}': {', '.join(invalid_args)}")
        return self
    ...
    def get_tool_info(self) -> dict[str, Any]:
        ...
        return {'name': self.tool_name, 'description': self.selected_tool.description, 'required_args': self.required_args, 'optional_args': self.optional_args, 'provided_args': list(self.tool_args.keys()), 'missing_args': list(set(self.required_args) - set(self.tool_args.keys())), 'schema': self.tool_schema, 'is_valid': self.is_tool_valid}
    ...
    @classmethod
    def create_from_tool(cls, tool: BaseTool, tool_args: dict[str, Any], available_tools: list[BaseTool], description: str | None=None, **kwargs) -> 'ToolStep':
        """Factory method to create ToolStep from a tool instance."""
        return cls(description=description or f'Execute {tool.name} tool', tool_name=tool.name, tool_args=tool_args, available_tools=available_tools, **kwargs)

def create_tool_steps_from_plan(tool_plan: list[dict[str, Any]], available_tools: list[BaseTool]) -> list[ToolStep]:
    """Create a list of ToolSteps from a plan description."""
    steps = []
    for i, step_info in enumerate(tool_plan):
        step = ToolStep(description=step_info.get('description', f'Step {i + 1}'), tool_name=step_info['tool_name'], tool_args=step_info.get('tool_args', {}), depends_on=step_info.get('depends_on', []), available_tools=available_tools)
        steps.append(step)
    return steps
...
def validate_tool_compatibility(tools: list[BaseTool]) -> dict[str, Any]:
    ...
    issues = {'duplicate_names': [], 'missing_schemas': [], 'tools_without_descriptions': [], 'valid_tools': []}
    seen_names = set()
    ...
    for tool in tools:
        ...
        if tool.args_schema and tool.description and (tool.description.strip() != '') and (tool.name not in issues['duplicate_names']):
