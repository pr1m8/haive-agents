...
class BasePlan(IntelligentStatusMixin, Generic[T]):
    ...
    def progress_percentage(self) -> float:
        ...
        in_progress = self._count_items_recursive(
            lambda x: hasattr(x, "status")
            and x.status in [TaskStatus.IN_PROGRESS, TaskStatus.PARALLEL_RUNNING]
        )
    ...
    def complexity_score(self) -> float:
        ...
        item_complexity = sum(
            getattr(item, "complexity_score", 0.1)
    ...
    def _is_executable_item(self, item: Any) -> bool:
        ...
        return (
            hasattr(item, "is_executable")
            or callable(item)
            or isinstance(item, str)
            or hasattr(item, "steps")  # Sub-plan
        )
...
class Task(IntelligentStatusMixin):
    ...
    def is_complete(self) -> bool:
        ...
        return (
            self.primary_plan
            and self.primary_plan.status == TaskStatus.COMPLETED
