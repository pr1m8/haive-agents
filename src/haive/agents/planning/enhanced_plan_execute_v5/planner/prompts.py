...
BASIC_PLANNING_TEMPLATE = ChatPromptTemplate.from_messages([
    ...
    ("human", """Please create a comprehensive plan for this objective:
...
RESEARCH_PLANNING_TEMPLATE = ChatPromptTemplate.from_messages([
    ...
    ("human", """Design a comprehensive research plan for this objective:
...
CREATIVE_PLANNING_TEMPLATE = ChatPromptTemplate.from_messages([
    ...
    ("human", """Design an innovative approach for this creative objective:
...
ADAPTIVE_PLANNING_TEMPLATE = ChatPromptTemplate.from_messages([
    ("system", STRATEGIC_PLANNER_SYSTEM_MESSAGE),
    ("human", """Create an adaptive plan considering previous attempts:
...
CONTEXTUAL_PLANNING_TEMPLATE = ChatPromptTemplate.from_messages([
    ("system", STRATEGIC_PLANNER_SYSTEM_MESSAGE),
    ("human", """Create a contextual plan with full situational awareness:
...
def get_planning_template(
    ...
    if domain_focus:
        ...
        if any(keyword in domain_lower for keyword in [
            ...
            "creative", "design", "writing", "art", "innovation", "brainstorm"
        ]):
    ...
    if any(keyword in objective_lower for keyword in [
        "research", "analyze", "investigate", "study", "find information"
    ]):
        return RESEARCH_PLANNING_TEMPLATE
    
    elif any(keyword in objective_lower for keyword in [
        "create", "design", "write", "brainstorm", "innovate", "develop"
    ]):
        return CREATIVE_PLANNING_TEMPLATE
...
def create_planning_context(
    ...
    return {
        ...
        "lessons_learned": "Apply lessons from previous attempts" if previous_attempts else "No previous lessons",
