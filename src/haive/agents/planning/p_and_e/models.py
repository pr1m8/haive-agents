...
class PlanStep(BaseModel):
    ...
    step_type: StepType = Field(default=StepType.ACTION, description='Type of step to help executor choose appropriate approach')
    dependencies: list[int] = Field(default_factory=list, description='List of step IDs that must be completed before this step')
    expected_output: str = Field(description='Description of the expected output/outcome from this step')
    status: StepStatus = Field(default=StepStatus.PENDING, description='Current status of the step')
    result: str | None = Field(default=None, description='Actual result/output from executing this step')
    error: str | None = Field(default=None, description='Error message if step failed')
    started_at: datetime | None = Field(default=None, description='Timestamp when step execution started')
    completed_at: datetime | None = Field(default=None, description='Timestamp when step execution completed')
...
class Plan(BaseModel):
    ...
    updated_at: datetime = Field(default_factory=datetime.now, description='Last time the plan was updated')
    ...
    def update_step_status(self, step_id: int, status: StepStatus, result: str | None=None, error: str | None=None) -> bool:
    ...
    def to_prompt_format(self) -> str:
        ...
        lines = [f'Objective: {self.objective}', f'Total Steps: {self.total_steps}', f'Progress: {len(self.completed_steps)}/{self.total_steps} ({self.progress_percentage:.1f}%)', '\nSteps:']
        for step in self.steps:
            status_marker = {StepStatus.COMPLETED: '✓', StepStatus.FAILED: '✗', StepStatus.IN_PROGRESS: '→', StepStatus.PENDING: '○', StepStatus.SKIPPED: '-'}.get(step.status, '?')
...
class ReplanDecision(BaseModel):
    ...
    decision: Literal['continue', 'replan', 'answer'] = Field(description='Decision on how to proceed')
    reasoning: str = Field(description='Explanation for the decision')
    final_answer: str | None = Field(default=None, description="Final answer if decision is 'answer'")
    replan_instructions: str | None = Field(default=None, description="Instructions for replanning if decision is 'replan'")
    skip_steps: list[int] | None = Field(default=None, description='Step IDs to skip if continuing with modifications')
...
class Act(BaseModel):
    ...
    action: Response | Plan = Field(description='Action to perform. If you want to respond to user, use Response. If you need to further use tools to get the answer, use Plan.')
