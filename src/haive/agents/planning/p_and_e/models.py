...
class PlanStep(BaseModel):
    ...
    step_id: int = Field(
    description="Unique identifier for this step (1-based index)")
    ...
    def serialize_datetime(self, dt: datetime | None -> Optional[str]:
    ...
    def is_ready(self) -> bool:
        ...
        """
        # This will be validated against actual plan in Plan model
...
class ReplanDecision(BaseModel):
    ...
    def validate_decision_fields(cls) -> "ReplanDecision":
        ...
        if self.decision == "replan" and not self.replan_instructions:
            raise ValueError(
                "replan_instructions required when decision is 'replan'")
        return self


# Union type for replan agent output
ReplanAction=Union[Plan, str]  # Either a new Plan or final answer string
