...
class PlanStep(BaseModel):
    ...
    step_id: int = Field(
    description="Unique identifier for this step (1-based)")
    description: str = Field(
        description="Clear, actionable description of what needs to be done"
    )
    expected_output: str = Field(
    description="What we expect to achieve from this step")
...
class ExecutionPlan(BaseModel):
    ...
    objective: str = Field(
    description="The main objective this plan aims to achieve")
...
class PlanEvaluation(BaseModel):
    ...
    def validate_decision_fields(self):
        ...
        if self.decision == "finalize" and not self.final_answer:
            raise ValueError(
                "final_answer required when decision is 'finalize'")
        if self.decision == "replan" and not self.revision_notes:
            raise ValueError(
                "revision_notes required when decision is 'replan'")
        return self
...
class RevisedPlan(BaseModel):
    ...
    description="The original objective (unchanged)")
    ...
    new_plan: ExecutionPlan = Field(
    description="The revised plan moving forward")
...
class PlanExecuteOutput(BaseModel):
    ...
    description="Summary of the execution process")
    ...
    description="Key findings from the execution")
