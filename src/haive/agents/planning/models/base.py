...
class BaseStep(BaseModel):
    ...
    This provides the core functionality that all step types share, while being flexible
    enough to support various planning patterns.
    """
    ...
    cls, v: list[Dependency]) -> list[Dependency]:
...
class BasePlan(BaseModel):
    ...
    This provides core planning functionality while allowing different planning
    strategies to extend and customize behavior.
    """
...
class AdaptivePlan(BasePlan):
    ...
    default=3, description="Maximum adaptation cycles")

    adaptation_count: int=Field(
    default=0, description="Current adaptation count")
