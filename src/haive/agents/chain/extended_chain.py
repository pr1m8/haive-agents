...
class ChainEdge(BaseModel):
    ...
    condition: Optional[Callable] = Field(default=None, description="Optional condition for edge traversal")
...
class ChainBuilder:
    ...
    def add_node(self, name: str, agent: Any, config: Optional[Dict[str, Any]] = None) -> 'ChainBuilder':
        """Add a node to the chain."""
        node = ChainNode(name=name, agent=agent, config=config or {})
        self.nodes.append(node)
        return self
    
    def add_edge(self, from_node: str, to_node: str, condition: Optional[Callable] = None) -> 'ChainBuilder':
