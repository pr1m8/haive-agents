...
import uuid
import os
from typing import Dict, List, Any, Optional, Union, Type, Callable, Sequence
...
from haive_agents.react_agent2.agent2 import ReactAgent
...
from haive_core.models.embeddings.base import EmbeddingsConfig
from haive_core.models.vectorstore.base import VectorStoreConfig, VectorStoreProvider
...
class DynamicReactAgentState(ReactAgentState):
    ...
    tool_call_id: Optional[str] = Field(
        default=None,
        description="Current tool call ID"
    )
...
class DynamicReactAgentConfig(ReactAgentConfig):
    ...
    tool_selection_node_name: str = Field(
        default="select_tools",
        description="Name for the tool selection node."
    )
    ...
    max_tools_per_turn: int = Field(
        default=5,
        description="Maximum number of tools to select per interaction."
    )
    
    # Override state schema to use our extended schema
    state_schema: Type[BaseModel] = Field(
        default=DynamicReactAgentState,
    ...
    tool_documents: Optional[List[Document]] = Field(
        default=None,
...
class DynamicReactAgent(ReactAgent):
    ...
    def setup_workflow(self) -> None:
        ...
        graph_builder.add_node(
            name=self.config.tools_node_name,
            config=tool_node
        )
        ...
        def has_tool_calls(state):
            ...
            # Dict format
                ...
                if "tool_calls" in last_message.get("additional_kwargs", {}) and last_message["additional_kwargs"]["tool_calls"]:
                    return True
            
            return False
        
        branch = Branch(
            function=lambda state: self.config.tools_node_name if has_tool_calls(state) else 
                     "structured_output_node" if self.config.response_format else END,
            destinations={
                self.config.tools_node_name: self.config.tools_node_name,
                "structured_output_node": "structured_output_node" if self.config.response_format else END,
                END: END
            },
            default=END
        )
        ...
        if self.config.repeat_selection:
            graph_builder.add_edge(self.config.tools_node_name, self.config.tool_selection_node_name)
        else:
        ...
        if self.vector_store_config:
            ...
            try:
                ...
                if not self.embeddings:
                    ...
                    logger.info(f"Created Azure OpenAI embeddings with model {embedding_model.model}")
                ...
                logger.info(f"Created vector store for tool selection using provided config")
    ...
    def _initialize_in_memory_vector_store(self, tool_documents: List[Document]):
        ...
        for tool_id, document in zip(self.tool_registry.keys(), tool_documents):
    ...
    def _create_tool_selection_function(self, state):
        ...
        try:
            tool_results = self.vector_store.similarity_search(
                query, 
                k=self.config.max_tools_per_turn
            )
            ...
            logger.info(f"Selected {len(selected_tool_ids)} tools: {', '.join(selected_tool_names)}")
    ...
    def _create_fixed_tool_node(self):
        ...
        def fixed_tool_node(state):
            ...
            if selected_tool_ids:
                ...
                selected_tools = [
                    tool_registry[tool_id] for tool_id in selected_tool_ids 
...
def create_dynamic_react_agent(
    ...
    tool_documents: Optional[List[Document]] = None,
    **kwargs
) -> DynamicReactAgent:
    """
