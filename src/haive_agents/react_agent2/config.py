...
from haive_agents.react_agent2.models import ReactState, Thought, Action
...
""",
    ...
    # Tools configuration
    tools: Union[Dict[str, Any], List[Any]] = Field(
    ...
    max_retry_attempts: int = Field(
        default=3,
        description="Maximum number of retry attempts per tool on failure."
    )
    
    # Model settings
    model: str = Field(
        default="gpt-4o",
        description="Model to use for thinking."
    )
    
    temperature: float = Field(
        default=0.7,
        description="Temperature for the thinking LLM."
    )
    
    # AugLLM for thinking (will be created in from_scratch)
    think_llm: Optional[AugLLMConfig] = None
    
    @classmethod
    def from_scratch(cls,
                    tools: Union[Dict[str, Any], List[Any]],
        ...
        if isinstance(tools, dict):
            for name, tool in tools.items():
                ...
                description = tool.__doc__ if hasattr(tool, "__doc__") and tool.__doc__ else f"Tool: {name}"
