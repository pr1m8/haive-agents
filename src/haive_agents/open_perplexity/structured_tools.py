...
class EnhancedGitHubIssuesLoader(GitHubIssuesLoader):
    ...
    loader_metadata = {
        ...
        "best_for": ["software development", "open source projects", "bug reports", "feature requests"],
        "source_type": "github",
...
class EnhancedHNLoader(HNLoader):
    ...
    loader_metadata = {
        ...
        "best_for": ["technology news", "tech community discussions", "startup information"],
        "source_type": "news",
        "structured_data": True,
        "parameters": ["story_id"]
    }
...
class DocumentLoaderRecommendationInput(BaseModel):
    ...
    research_question: Optional[str] = Field(None, description="Specific research question")
    data_types: Optional[List[str]] = Field(None, description="Types of data needed (web, academic, news, etc.)")
...
class ArxivLoaderInput(BaseModel):
    ...
    load_all_available_meta: bool = Field(True, description="Load all available metadata for papers")
...
def recommend_document_loaders(research_topic: str, 
...
def load_recursive_web(url: str, max_depth: int = 2, prevent_outside: bool = True) -> Dict[str, Any]:
...
def load_arxiv_papers(query: str, max_results: int = 5, load_all_available_meta: bool = True) -> Dict[str, Any]:
    ...
    try:
        ...
        for doc in docs:
            ...
            content_preview = doc.page_content[:1500] + "..." if len(doc.page_content) > 1500 else doc.page_content
...
def load_github_issues(repo: str, access_token: Optional[str] = None, state: str = "open") -> Dict[str, Any]:
    """Load issues from a GitHub repository"""
    try:
        if access_token:
            os.environ["GITHUB_TOKEN"] = access_token
        
        loader = EnhancedGitHubIssuesLoader(
            repo=repo,
            state=state
        )
        docs = loader.load()
...
try:
    class TavilySearchInput(BaseModel):
        ...
        search_depth: Optional[str] = Field("basic", description="Search depth (basic or comprehensive)")
