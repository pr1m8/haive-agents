...
from haive_agents.sequence.storm import STORMAgentConfig, STORMAgent
...
async def run_storm_agent(
    ...
    try:
        ...
        logger.error(f"Error running STORM agent: {str(e)}")
        raise

def main():
    """Main entry point for the script."""
    parser = argparse.ArgumentParser(description="Generate a Wikipedia-style article using the STORM agent")
    parser.add_argument("topic", help="Topic to research and write about")
    parser.add_argument("--output", "-o", help="Output file path (optional)")
    parser.add_argument("--perspectives", "-p", type=int, default=3, help="Number of perspectives (default: 3)")
    parser.add_argument("--turns", "-t", type=int, default=5, help="Max conversation turns (default: 5)")
