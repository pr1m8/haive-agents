...
from haive_agents.self_discover.config import SelfDiscoverAgentConfig
from haive_agents.self_discover.state import SelfDiscoverState
from haive_agents.self_discover.models import (
    ...
    def setup_workflow(self) -> None:
        ...
        def select_modules(state: SelfDiscoverState) -> Command:
            ...
            try:
                ...
                logger.error(f"Error in select_modules: {str(e)}")
                return Command(
                    update={"error": f"Error in module selection: {str(e)}"},
                    goto=END
                )
        ...
        def adapt_modules(state: SelfDiscoverState) -> Command:
            ...
            try:
                ...
                logger.error(f"Error in adapt_modules: {str(e)}")
                return Command(
                    update={"error": f"Error in module adaptation: {str(e)}"},
                    goto=END
                )
        ...
        def create_structure(state: SelfDiscoverState) -> Command:
            ...
            try:
                ...
                logger.error(f"Error in create_structure: {str(e)}")
                return Command(
                    update={"error": f"Error in structure creation: {str(e)}"},
                    goto=END
                )
        ...
        def execute_reasoning(state: SelfDiscoverState) -> Command:
            ...
            try:
                ...
                logger.error(f"Error in execute_reasoning: {str(e)}")
                error_msg = f"Error in reasoning execution: {str(e)}"
...
def create_self_discover_agent(
    ...
    reasoning_modules: Optional[List[str]] = None,
    select_prompt: Optional[Union[str, ChatPromptTemplate]] = None,
    adapt_prompt: Optional[Union[str, ChatPromptTemplate]] = None,
    structure_prompt: Optional[Union[str, ChatPromptTemplate]] = None,
    reasoning_prompt: Optional[Union[str, ChatPromptTemplate]] = None,
    visualize: bool = True,
    **kwargs
) -> SelfDiscoverAgent:
    """
