...
from haive_agents.simple.agent import SimpleAgent, SimpleAgentConfig, SimpleAgentSchema
...
from haive_core.graph.routing import RoutingCondition, create_router
...
class RoutingAgentConfig(SimpleAgentConfig):
    ...
    handlers: Dict[str, Union[AugLLMConfig, Callable]] = Field(
        default_factory=dict,
        description="Handler nodes (LLMs or functions)"
    )
    
    # Routing conditions
    conditions: Dict[str, List[Callable]] = Field(
        default_factory=dict,
        description="Routing conditions by source node"
    )
    
    # Default routes
    default_routes: Dict[str, str] = Field(
        default_factory=dict,
        description="Default routes when no conditions match"
    )
...
def create_routing_agent(
    ...
    handlers: Dict[str, Union[AugLLMConfig, Callable]],
...
if __name__ == "__main__":
    ...
    def route_to_question_handler(state):
        ...
        return "?" in message or message.lower().startswith("what") or message.lower().startswith("how")
    ...
    print(f"Question result: {result.get('route_history', [])} -> {result.get('output', '')}")
    
    # Run with a task
    result = agent.run("Can you help me create a workout plan?")
    print(f"Task result: {result.get('route_history', [])} -> {result.get('output', '')}")