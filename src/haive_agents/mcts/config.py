...
class MCTSAgentConfig(AgentConfig):
    ...
    candidates_per_rollout: int = Field(default=5, description="Number of candidates to generate per rollout")
    exploration_weight: float = Field(default=1.0, description="Exploration weight for UCB")
    
    # Prompts
    initial_prompt_template: Optional[ChatPromptTemplate] = Field(
        default=None, description="Template for initial response"
    )
    expansion_prompt_template: Optional[ChatPromptTemplate] = Field(
        default=None, description="Template for candidate expansion"
    )
    reflection_prompt_template: Optional[ChatPromptTemplate] = Field(
