...
async def type_text(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"message": f"Typed '{text_content}' in element {bbox_id}", "page_url": page_url}
...
async def scroll(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"message": f"Failed to scroll: {str(e)}"}
    
    page_url = page.url
    return {"message": f"Scrolled {direction} in {'window' if target.upper() == 'WINDOW' else 'element'}", "page_url": page_url}
...
async def to_google(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"message": "Navigated to google.com.", "page_url": "https://www.google.com/"}