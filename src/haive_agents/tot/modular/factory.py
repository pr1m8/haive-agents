...
from typing import Optional, Callable, Dict, Any, List, Union, Type
...
from haive_agents.tot.modular.agent import ToTAgent
from haive_agents.tot.modular.config import ToTAgentConfig
from haive_agents.tot.modular.state import Candidate, ToTState
...
def create_tot_agent(
    ...
    expand_prompt: Optional[Union[str, ChatPromptTemplate]] = None,
    score_prompt: Optional[Union[str, ChatPromptTemplate]] = None,
    ...
    def score_equation(problem: str, solution: str) -> tuple:
        ...
        try:
            ...
            if abs(result - 24) < 0.0001:
                return 1.0, f"Perfect! The expression equals exactly 24"
            else:
                return proximity, f"Expression evaluates to {result}, which is {abs(24 - result)} away from 24"
        except Exception as e:
            return 0.05, f"Error evaluating the formula: {str(e)}"
