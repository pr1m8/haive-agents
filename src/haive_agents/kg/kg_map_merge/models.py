...
class EntityRelationship(BaseModel):
    ...
    # Additional fields for enhanced relationship representation
    ...
    @classmethod
    def from_graph_relationship(cls, relationship: Relationship, confidence_score: float = 0.5, supporting_evidence: Optional[str] = None):
        """
    ...
    def add_relationship(self, relationship: EntityRelationship):
        ...
        if not any(
            (existing.source == relationship.source and 
             existing.target == relationship.target and 
...
def main():
    ...
    person_node = EntityNode(id="marie_curie", type="Scientist", properties={
        "name": "Marie Curie",
        "nationality": "Polish-French",
        "field": "Physics and Chemistry"
    })
    
    country_node = EntityNode(id="poland", type="Country", properties={
        "name": "Poland",
        "continent": "Europe"
    })
    
    # Create an example relationship
