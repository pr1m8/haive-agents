...
from typing import Any, Dict, List, Optional, Type, Union, Callable, Sequence
import logging
...
class ReactAgent(SimpleAgent):
    ...
    def __init__(self, config: ReactAgentConfig):
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Error creating RetryPolicy: {str(e2)} - using minimal configuration")
    ...
    def _prepare_tools(self, tools_input):
        ...
        logger.info(f"Prepared {sum(len(tools) for tools in tools_map.values())} tools across {len(tools_map)} nodes")
        return tools_map
    ...
    def setup_workflow(self) -> None:
        ...
        logger.info(f"Set up ReactAgent workflow for {self.config.name} with {len(self.tool_nodes)} tool nodes")
        ...
        if messages:
            ...
            if isinstance(last_message, AIMessage):
                ...
                if content and any(marker in content for marker in [
                    ...
                    return "structured_output" if getattr(self.config, 'use_structured_output_node', False) else "end"
