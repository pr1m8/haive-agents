...
from haive_agents.v2.state import ReactAgentState
from haive_agents.simple.config import SimpleAgentConfig
import logging
logger = logging.getLogger(__name__)

ToolsInput = Union[
    List[Union[BaseTool, StructuredTool, Tool, Type[BaseModel], Callable]],
    Dict[str, Union[BaseTool, StructuredTool, Tool, Type[BaseModel], Callable, List[Union[BaseTool, StructuredTool, Tool, Type[BaseModel], Callable]]]]
]
...
class ReactAgentConfig(SimpleAgentConfig):
    ...
    tool_choice: Optional[str] = Field(
        default=None, 
        description="Force the agent to use a specific tool."
    )
    ...
    max_retries: int = Field(
        default=3,
        description="Maximum number of retries for tool execution failures."
    )
    ...
    parallel_tool_execution: bool = Field(
        default=False,
        description="Whether to execute multiple tool calls in parallel."
    )
    ...
    agent_node_name: str = Field(
        default="agent",
        description="Name for the agent (LLM) node"
    )
    
    tools_node_prefix: str = Field(
        default="tool_",
        description="Prefix for auto-generated tool node names"
    )
