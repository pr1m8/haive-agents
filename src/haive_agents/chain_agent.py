...
from typing import Any, Dict, List, Optional, Union, Type, Sequence, Annotated
import logging
import json
...
from haive_agents.simple.agent import SimpleAgent, SimpleAgentConfig, SimpleAgentStateSchema
...
class ChainAgentSchema(SimpleAgentStateSchema):
    ...
    output: Optional[str] = Field(
        default="",
        description="Final output from the chain"
    )
    intermediate_results: Dict[str, Any] = Field(
        default_factory=dict,
        description="Results from each engine in the chain"
    )
    current_step: int = Field(
        default=0,
        description="Current step in the chain"
    )
    chain_data: Dict[str, Any] = Field(
        default_factory=dict,
        description="Data passed between chain steps"
    )
...
class ChainAgentConfig(SimpleAgentConfig):
    ...
    # Step configuration
    step_names: Optional[List[str]] = Field(
        default=None,
    ...
    state_schema: Type[BaseModel] = Field(
        default=ChainAgentSchema,
        description="Schema for the agent state"
    )
    
    @classmethod
    def from_engines(cls, 
...
class ChainAgent(SimpleAgent):
    ...
    def setup_workflow(self) -> None:
        ...
        if len(step_names) != len(self.config.engines):
            logger.warning(f"Number of step names ({len(step_names)}) does not match number of engines ({len(self.config.engines)})")
        ...
        for i, (step_name, engine) in enumerate(zip(step_names, self.config.engines)):
        ...
        logger.info(f"Set up chain workflow for {self.config.name} with {len(self.config.engines)} steps")
            ...
            try:
                ...
                if step_idx == 0:
                    ...
                    if "input_text" in chain_data:
                        ...
                        if hasattr(engine_config, 'prompt_template') and hasattr(engine_config.prompt_template, 'input_variables'):
                ...
                logger.error(f"Error in step {step_idx} ({step_name}): {str(e)}")
                error_msg = f"Error in step {step_idx} ({step_name}): {str(e)}"
