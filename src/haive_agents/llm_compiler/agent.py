...
from haive_core.engine.agent.agent import AgentArchitecture
from haive_core.engine.aug_llm import compose_runnable

from haive_agents.llm_compiler.models import (
    #CompilerState, 
    CompilerPlan, 
    CompilerStep
)
from haive_agents.llm_compiler.models import JoinerOutput, FinalResponse, Replan
from haive_agents.llm_compiler.output_parser import LLMCompilerPlanParser
from haive_agents.llm_compiler.config import LLMCompilerAgentConfig
from haive_agents.llm_compiler.state import CompilerState
...
class LLMCompilerAgent(AgentArchitecture):
    ...
    def _create_fallback_plan(self, query: str) -> CompilerPlan:
        ...
        # Find a search tool
        search_tool = next((tool.name for tool in self.tools if "search" in tool.name.lower()), None)
    ...
    @staticmethod
    def _execute_step(step: CompilerStep, results: Dict[int, Any], tool_map: Dict[str, BaseTool]) -> Any:
        """
    ...
    def run(self, query: str):
        ...
        final_state = self.app.ainvoke(initial_state, config=self.config.runnable_config,debug=True)
    ...
    async def arun(self, query: str):
        ...
        final_state = await self.app.ainvoke(initial_state, config=self.config.runnable_config,debug=True)
...
def main():
    ...
    asyncio.run(agent.arun("Find the winner of the most recent wimbldedon championship, and write a python program printing their name and execute it."))    
