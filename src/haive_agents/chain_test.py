...
from haive_agents.simple.chain_agent import create_chain_agent, ChainAgentSchema
...
class DocumentClassification(BaseModel):
    ...
    document_type: str = Field(description="Type of document (legal, financial, technical, creative, academic, news)")
    ...
    complexity_level: str = Field(description="Complexity level (simple, moderate, complex, technical)")
    target_audience: str = Field(description="Intended audience for the document")
    sensitive_content: bool = Field(description="Whether document contains sensitive/confidential info")
...
class BranchingDocumentAnalyzer:
    ...
    def _setup_engines(self):
        ...
        document_classifier_prompt = ChatPromptTemplate.from_messages([
            ('system', "You are a document classification expert. Analyze the provided text to determine document type and characteristics."),
            ('human', "Analyze the following document and classify its type, subject, complexity, and other characteristics:\n\n{text}")
        ])
        ...
        router_prompt = ChatPromptTemplate.from_messages([
            ('system', "You are a document routing specialist. Based on document classification, determine the appropriate analysis path."),
            ('human', "Based on this document classification, determine which specialized analysis path should be used.\nClassification: {classification}\n\nRespond with ONLY ONE of these exact words: legal, financial, technical, academic, general")
        ])
        ...
        legal_analyzer_prompt = ChatPromptTemplate.from_messages([
            ...
            ('human', "Perform a detailed legal analysis of this document, identifying parties, clauses, obligations, risks, and other legal aspects:\n\n{text}")
        ])
        ...
        financial_analyzer_prompt = ChatPromptTemplate.from_messages([
            ...
            ('human', "Perform a detailed financial analysis of this document, identifying entities, metrics, trends, risks, and opportunities:\n\n{text}")
        ])
        ...
        technical_analyzer_prompt = ChatPromptTemplate.from_messages([
            ...
            ('human', "Perform a detailed technical analysis of this document, identifying technologies, methodologies, requirements, specifications, and limitations:\n\n{text}")
        ])
        ...
        academic_analyzer_prompt = ChatPromptTemplate.from_messages([
            ...
            ('human', "Perform a detailed academic analysis of this document, identifying research field, methodologies, findings, and scholarly contribution:\n\n{text}")
        ])
        ...
        general_analyzer_prompt = ChatPromptTemplate.from_messages([
            ...
            ('human', "Provide a comprehensive analysis of this document, covering its main points, structure, and significance:\n\n{text}")
        ])
    ...
    def build_chain(self):
        ...
        class BranchingChainSchema(ChainAgentSchema):
            ...
            specialized_analysis: str = Field(default="", description="Results from specialized analysis")
            chain_data: Dict[str, Any] = Field(default_factory=dict, description="Data passed between chain steps")
        
        # Create a dynamic graph
        dg = DynamicGraph(
            components=list(self.engines.values()),
            state_schema=BranchingChainSchema
        )
        ...
        def classifier_node(state):
            ...
            try:
                # Get input text
                input_text = ""
                if hasattr(state, 'chain_data') and isinstance(state.chain_data, dict) and "input_text" in state.chain_data:
                ...
                logger.error(f"Error in document classifier: {str(e)}")
                return {
                    "error": f"Error in document classifier: {str(e)}"
                }
        ...
        def router_node(state):
            ...
            try:
                ...
                classification = "{}"
                if hasattr(state, 'chain_data') and isinstance(state.chain_data, dict) and "document_classifier" in state.chain_data:
                ...
                if hasattr(state, 'intermediate_results') and isinstance(state.intermediate_results, dict):
                ...
                logger.error(f"Error in document router: {str(e)}")
                return {
                    "error": f"Error in document router: {str(e)}",
                    "branch_path": "general",
        ...
        def create_analyzer_node(analyzer_name):
            def analyzer_node(state):
                ...
                try:
                    ...
                    error_msg = f"Error in {analyzer_name}: {str(e)}"
                    return {
                        "error": error_msg,
                        "specialized_analysis": f"Analysis failed with error: {str(e)}",
                        "__command__": {"goto": "executive_summary"}
                    }
        ...
        def summary_node(state):
            ...
            try:
                # Get required fields
                document_type = state.document_type if hasattr(state, 'document_type') else "Unknown"
                primary_topic = state.primary_topic if hasattr(state, 'primary_topic') else "Unknown"
                specialized_analysis = state.specialized_analysis if hasattr(state, 'specialized_analysis') else "{}"
    ...
    def analyze_document(self, document_text):
        ...
        # Create agent config
        from haive_agents.simple.chain_agent import ChainAgentConfig
