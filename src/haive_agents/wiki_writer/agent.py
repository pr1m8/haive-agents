...
class WikiWriterAgentConfig(AgentArchitectureConfig):
    ...
    aug_llm_config: AugLLMConfig = Field(default=wiki_writer_aug_llm, description="LLM config for Wiki Writer")
    state_schema: WikiWriterState = Field(default=WikiWriterState, description="State schema for the agent")
