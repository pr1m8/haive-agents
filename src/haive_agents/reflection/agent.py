...
from haive_agents.reflection.config import ReflectionAgentConfig
...
class ReflectionAgent(SimpleAgent):
    ...
    def setup_workflow(self) -> None:
        ...
        if self.config.reflection.reflection_llm and self.config.reflection.reflection_llm != self.config.engine:
    ...
    def _create_reflection_function(self):
        ...
        def reflection_function(state: ReflectionAgentState) -> Dict[str, Any]:
            try:
                ...
                if reflection.found_solution or reflection_round >= self.config.reflection.max_reflection_rounds:
                    return Command(
                        update=state_update,
                        goto="end"
                    )
    ...
    def _create_improvement_function(self):
        ...
        def improvement_function(state: ReflectionAgentState) -> Dict[str, Any]:
            try:
                ...
                replaced = False
                for msg in messages:
                    if not replaced and (isinstance(msg, AIMessage) or getattr(msg, 'type', None) == 'ai'):
    ...
    def _create_evaluation_function(self):
        ...
        def evaluation_function(state: ReflectionAgentState) -> Dict[str, Any]:
            ...
            if reflection_score >= auto_threshold:
                logger.info(f"Response quality meets auto-accept threshold: {reflection_score} >= {auto_threshold}")
                return Command(
                    update=state,
                    goto="end"
                )
            ...
            logger.info(f"Continuing reflection: round {reflection_round}, score {reflection_score}")
            return Command(
                update=state,
                goto="continue"
            )
